...
const FAIL = [];
const WARN = [];
function fail(msg) { FAIL.push(msg); console.error(`  ✗ ${msg}`); }
function warn(msg) { WARN.push(msg); console.warn(`  ! ${msg}`); }
...
const wsPkgDirs = [
  ...fs.readdirSync(path.join(repoRoot, "packages"), { withFileTypes: true })
    .filter((e) => e.isDirectory()).map((e) => `packages/${e.name}`),
  ...fs.readdirSync(path.join(repoRoot, "apps"), { withFileTypes: true })
    .filter((e) => e.isDirectory()).map((e) => `apps/${e.name}`),
];
...
for (const rel of wsPkgDirs) {
  ...
  const srcFiles = listFiles(path.join(repoRoot, rel, "src"), [".ts", ".tsx", ".mts", ".cts", ".js", ".mjs", ".jsx"]);
  ...
  if (undeclared.size > 0) {
    fail(`${rel}: imports not in package.json — ${[...undeclared].sort().join(", ")}`);
    undeclaredCount += undeclared.size;
  }
}
if (undeclaredCount === 0) ok(`all ${wsPkgDirs.length} workspace packages have all imports declared`);

// ── 3. plugins/ structure ─────────────────────────────────────────
console.log("\n[3/6] plugins/<id>/ structure (verify-release sentinels)");
const isPluginManifest = (f) => /^PLUGIN(\.[a-z-]+)?\.md$/i.test(f);
const pluginDirs = fs.readdirSync(path.join(repoRoot, "plugins"), { withFileTypes: true })
...
for (const entry of pluginDirs) {
  ...
  const hasRoot = fs.existsSync(dir) && fs.readdirSync(dir).some(isPluginManifest);
  let hasNested = false;
  const runtimesDir = path.join(dir, "runtimes");
  if (fs.existsSync(runtimesDir)) {
    hasNested = fs.readdirSync(runtimesDir, { withFileTypes: true })
      .filter((e) => e.isDirectory())
      .some((sub) => fs.readdirSync(path.join(runtimesDir, sub.name)).some(isPluginManifest));
  }
...
if (pluginIssues === 0) ok(`all ${pluginDirs.length} plugins have package.json + PLUGIN.md`);

// ── 4. worlds/ structure ──────────────────────────────────────────
console.log("\n[4/6] worlds/<id>/ structure");
const worldDirs = fs.readdirSync(path.join(repoRoot, "worlds"), { withFileTypes: true })
  .filter((e) => e.isDirectory());
let worldIssues = 0;
for (const entry of worldDirs) {
  ...
  const hasLore = fs.readdirSync(dir).some((f) => /^WORLD(\.[a-z-]+)?\.md$/i.test(f));
  if (!hasLore) {
    fail(`worlds/${entry.name}: missing WORLD*.md`);
    worldIssues++;
  }
}
if (worldIssues === 0) ok(`all ${worldDirs.length} worlds have world.yaml + WORLD*.md`);
...
try {
  ...
  try {
    execSync("actionlint .github/workflows/*.yml", { cwd: repoRoot, stdio: "pipe" });
...
console.log("  Safe to: git tag -a v0.0.x -m '...' && git push origin v0.0.x\n");
