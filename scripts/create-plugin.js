...
if (withTools && (targetArg || runtimesArg)) {
  console.error('错误：--with-tools 是旧式单 runtime 模式，不能与 -t / -r 同用。');
  process.exit(1);
}
...
const targetBaseDir = withTools ? resolve(ROOT, 'plugins') : resolve(targetArg ?? DEFAULT_TARGET);
...
function resolveRepoPackageManager() {
  try {
    const repoPkg = JSON.parse(readFileSync(resolve(ROOT, 'package.json'), 'utf-8'));
    if (typeof repoPkg.packageManager === 'string' && repoPkg.packageManager.length > 0) {
...
if (mode === 'legacy-with-tools') {
  console.log(`  1. 编辑 ${relative(process.cwd(), targetDir)}/PLUGIN.md，填写 LLM 提示词`);
  console.log(`  2. 修改 tools/example.js，实现工具逻辑`);
  console.log(`  3. 在 ${relative(process.cwd(), targetDir)} 下跑 pnpm install && pnpm test`);
} else {
  console.log(`  1. 检查 ${relative(process.cwd(), targetDir)}/runtimes/<name>/PLUGIN.md，按需修改`);
  if (mode === 'demo-multi-runtime') {
    console.log('  2. 默认包含 echo (function) + summarizer (agent) 两个 runtime');
    console.log('  3. 启动框架后侧栏会出现 Hello World 标签页，按按钮验证');
  } else {
    console.log('  2. 函数 runtime 编辑 handler.js，agent runtime 编辑 PLUGIN.md system prompt');
  }
  if (targetBaseDir !== DEFAULT_TARGET) {
    console.log(`  4. 如未在默认插件目录，请确认 COVEL_PLUGINS_DIR 指向 ${targetBaseDir}`);
  }
}
...
function parseRuntimeSpec(spec) {
  ...
  return items.map((item) => {
    ...
    if (!rawName || !/^[a-z][a-z0-9-]*$/.test(rawName)) {
      console.error(`错误：runtime 名称 "${rawName}" 不合法（小写字母、数字、连字符，字母开头）。`);
      process.exit(1);
    }
    if (seen.has(rawName)) {
    ...
    if (!VALID_RUNTIME_TYPES.has(type)) {
      console.error(`错误：runtime 类型 "${type}" 不合法，必须是 function 或 agent。`);
      process.exit(1);
    }
...
function runCustomMultiRuntime(runtimes) {
  ...
  if (!existsSync(baseTemplateDir)) {
    console.error('错误：找不到 templates/plugin-multi-runtime/ —— 用 git status 确认未被删除。');
    process.exit(1);
  }
  ...
  for (const entry of readdirSync(baseTemplateDir)) {
    ...
    if (stat.isDirectory()) {
      ...
      writeFileSync(destPath, replacePlaceholders(readFileSync(srcPath, 'utf-8')), 'utf-8');
    }
  }
  ...
  for (const rt of runtimes) {
    ...
    if (rt.type === 'function') {
      writeFileSync(join(rtDir, 'PLUGIN.md'), renderFunctionStubManifest(rt.name), 'utf-8');
      writeFileSync(join(rtDir, 'handler.js'), renderFunctionStubHandler(rt.name), 'utf-8');
    } else {
      writeFileSync(join(rtDir, 'PLUGIN.md'), renderAgentStubManifest(rt.name), 'utf-8');
    }
  }
}
...
function printUsage() {
  console.log('用法：');
  console.log('  node scripts/create-plugin.js <plugin-name>                  # 默认 hello-world');
  console.log('  node scripts/create-plugin.js <plugin-name> -t <dir>         # 自定义目标');
  console.log('  node scripts/create-plugin.js <plugin-name> -r foo:function  # 自定义 runtime');
  console.log('  node scripts/create-plugin.js <plugin-name> --with-tools     # 旧式单 runtime');
  console.log('');
  console.log('选项：');
  console.log('  -t, --target <dir>    目标目录，默认 ~/.covel/plugins');
  console.log('  -r, --runtimes <list> 逗号分隔的 runtime 列表，每项为 name 或 name:type');
  console.log('                        type ∈ { function, agent }，默认 agent');
  console.log('  --with-tools          旧式单 runtime + tools/，目标固定为 <repo>/plugins/');
}
