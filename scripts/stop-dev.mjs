...
const runAndCollect = async (command, args, { allowEmpty = false, spawnImpl = spawn } = {}) =>
...
export const collectCandidatePids = (processes, repoPids, listeningPids = new Set(), repoDir = "") => {
  ...
  for (const processInfo of processes) {
    const repoRelated = repoPids.has(processInfo.pid) || (repoDir !== "" && processInfo.command.includes(repoDir));
    const listening = listeningPids.has(processInfo.pid);

    if ((repoRelated && matchesDevCommand(processInfo.command)) || (repoRelated && listening)) {
...
export const selectRootPids = (processes, candidates) =>
  processes.filter(({ pid, ppid }) => candidates.has(pid) && !candidates.has(ppid)).map(({ pid }) => pid);
...
export const stopDevProcesses = async (
  ...
  const candidates = collectCandidatePids(processes, repoPids, listeningPids, repoDir);
  ...
  const remainingCandidates = collectCandidatePids(remainingProcesses, remainingRepoPids, remainingListeningPids, repoDir);
  const remainingRootPids = selectRootPids(remainingProcesses, remainingCandidates);
...
export const main = async (cwd = process.cwd()) => {
  ...
  if (remainingRootPids.length > 0) {
    console.error(`Failed to stop dev process roots: ${remainingRootPids.join(", ")}`);
    return 1;
  }

  console.log(`Stopped ${rootPids.length} frontend/backend dev process root(s).`);
  return 0;
};
