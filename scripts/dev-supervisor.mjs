  ...
  new Promise((resolve, reject) => {
    ...
    child.once("exit", (code) => {
      if (
        ...
        stderr.includes("There is no running instance")
      ) {
        resolve(true);
        return;
      }
...
export const terminateProcessTree = async (
  rootPid,
  {
    ...
    terminateProcessTreeWindows:
      terminateProcessTreeWindowsImpl = terminateProcessTreeWindows,
  ...
  log(
    `Process ${rootPid} did not exit after ${gracefulWaitMs}ms; forcing shutdown.`,
  );
