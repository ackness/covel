...
- For live AI world generation, confirm the active provider config through `/api/config/info`; the dev server can use `/Users/wuyong/.covel/llm.toml` instead of the repo-local `llm.toml`. A failed `/api/ai/ping` without browser provider headers is not enough to prove generation auth failure; check `logsDir/server.log` for `ai/generate-world`, `LLM responded`, parse, validation, and timeout entries.
- AI world generation streams provider output when the adapter exposes `stream()`, then falls back to `generate()`. Each retry gets its own provider timeout. If logs show a large `contentLength` followed by parse failures, the model likely omitted the `===WORLD_YAML===` / `===WORLD_MD===` delimiters or returned code fences; treat that as an output-format issue and inspect the prompt/parser before raising the timeout.
- Generated worlds now keep the model prompt small and server-normalize the package into current worldData format: `world.yaml` points at `data/world.data.yaml`, and that descriptor imports `data/dimensions.yaml` into `world:metadata.dimensions`. E2E checks can assert both `metadata.dimensions` and `metadata.worldData.sources[].target === "world:metadata.dimensions"`.
- Generated worlds support three save targets. `server-file` persists a world package under the server worlds directory, `server-store` saves the normalized `WorldRecord` through `packages/store`, and `return-only` streams the record back without saving it on the server. Use `return-only` for live HTTP tests that should not leave persistent worlds behind.
- Browser local storage mode uses `return-only` and then saves the returned world through `packages/store` IndexedDB. Storage assertions can check `world.metadata.storage.scope === "browser"` and `world.metadata.storage.backend === "indexeddb"` after the UI list refreshes. Remote PostgreSQL/SQLite persistence uses `server-store` and should show `scope === "server"` with `backend === "sqlite"` or `"pg"`. Legacy file persistence shows `backend === "file"`.
- If `/api/health` reports `storeBackend: "memory"`, the web app chooses local IndexedDB for user data because server memory is ephemeral. If it reports `"sqlite"` or `"pg"`, the web app uses the server store as authoritative.
- Live generation can succeed on a later validation retry after roughly 190 seconds. Browser e2e should wait for the done state or world-list growth for at least 240 seconds before reporting a timeout.
- Raw `/api/worlds` responses keep file-loaded dimensions under `metadata.dimensions`; the web API client maps that to top-level `dimensions`. E2E tests that call HTTP directly should read `world.dimensions ?? world.metadata?.dimensions`.
