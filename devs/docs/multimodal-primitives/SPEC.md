...
**旧 inline 数据处理方式**（2026-04-26 用户决策）：历史 base64 / data URI 数据采用一次性重置策略，新版本生成结果全部通过 `MediaRef` 写入 MediaStore。开发机直接删除 `/Users/wuyong/.covel/data/covel.db*` 和旧插件 `.bak-p0c-*` 目录，启动后由当前 schema 重建 DB。

#### 清理后的后果

- 旧 session、旧插件数据和旧 inline 图片全部从空库重新开始
- 已发布插件需要按收紧后的 `asset.generate` schema 改写（`{ ref: MediaRef, modality, meta }`）——提供 codemod
- **回滚边界**：本地 DB 重置后，历史 session 与历史图片字节全部清空；回滚旧版需要重新创建测试数据
...
### P0-d：snapshot/fork 媒体引用 + 端到端回归（1 周）

- 本地开发环境删除旧 `covel.db*`，从当前 schema 重建空库
