...
export interface AuditRuntime {
   ...
   * 输出：每个冲突字段的最终值和裁决理由
   */
  resolve(conflicts: WriteConflict[], turnContext: TurnContext): Promise<AuditDecision[]>;
}
...
export interface RuntimeErrorHandler {
   ...
   */
  handle(error: RuntimeError, retryCount: number, policy: RetryPolicy): ErrorAction;
}
