...
`generate-scenes.mjs` 通过 `@covel/ai-provider/image-wires` 等公开子入口使用框架能力；这些 workspace 入口解析为 TypeScript，需要用 **tsx** 运行。`emit-scenes.mjs` 是纯 Node 脚本，`node` 直接跑即可。生成方式与 provider 接线（llm.toml slot / keys.env / image wire）与立绘完全同一套机制，不重复展开，见 [角色立绘生成指南](./world-portraits.md#生成方式框架统一-image-wire)。
