...
只有指定目标被改为初始手动触发，加入协作插件不会自动运行其全部 runtime；声明的事件下游仍可执行。跨插件 `needs` 需要实际的上游结果，单纯加载包并不制造结果。仅调试不依赖上游的路径时，可以用 case `"ignoreUpstreams": true` 或 CLI `--ignore-upstreams`。报告的 `pluginData` 仍只包含被测插件。
