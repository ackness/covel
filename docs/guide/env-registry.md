...
- `TRUSTED_PROXY_IPS`、`COVEL_LLM_REPLAY`、`COVEL_LLM_REPLAY_DIR` 目前标记为 `documented`，后续实现可以直接提升为 `active`。（`COVEL_ALLOWED_LLM_HOSTS` 已移除：SSRF guard 设计上即 open-by-default，从未读取该变量，也无实现计划。）
