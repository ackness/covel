...
Returns assets owned by `sessionId` whose `meta` contains every key/value in `filter` — an exact, shallow subset match, one shared implementation (`filterAssetsByMetadata` in `packages/store/src/media-store/filter.ts`) reused by all four backends. A `filter` value of `undefined` matches both a missing key and a key explicitly stored as `undefined` (`meta[k] === v` — both sides read `undefined`). Only primitive values (string/number/boolean/`null`) compare meaningfully; objects and arrays never match because `===` on them is reference equality. Result sets are expected to stay small — per-session media volume is tens of records, so this is a full scan (`ponytail:` comment in `filter.ts` — push down to SQL when volume outgrows that). `filter.ts` is kept free of Node built-ins (unlike `utils.ts`) so the browser-reachable IndexedDB backend can import it without pulling `node:crypto`/`node:path` into the web bundle.
