...
- Plugins register additional wires from their `entry` module via
  `covel.registerWires({ image?, speech?, transcription? })` — the `wires`
  PLUGIN.md frontmatter field still loads but is deprecated
  (ids auto-namespaced `<pluginId>/<wireId>`; trust-gated loading) — see
  [plugin-authoring-advanced.md](../guide/plugin-authoring-advanced.md#注册自定义-wireentry-里的-covelregisterwires).
