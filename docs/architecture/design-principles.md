...
让任何创作者——会写代码的、不会写代码的、或一个 AI Agent——把一个游戏创意按规范写成声明，框架就能编排成可玩的场景。**规范约束"怎么接进框架"，不约束"能玩什么"。** 一次赛博潜入、一场法庭辩论、一回 galgame 表白，对框架是同一件事：一组按契约声明好的插件和 [World](../glossary.md)，被 [Kernel](../glossary.md) 一回合一回合编排。
