...
Wired and tested but not yet consumed by the retriever. Available for
Phase 3.5 once the function-runtime context can access the gateway.
...
tests (8 new embedding path tests for the loader / openai-chat
dispatch / gateway routing).
...
1. Extend `FunctionHandlerContext` (in
