...
export class ToolValidationError extends Error {
  ...
  constructor(zodError: ZodError) {
    ...
    const issues = (zodError as unknown as { issues?: Array<{ path: (string | number)[]; message: string }> }).issues
      ?? (zodError as unknown as { errors?: Array<{ path: (string | number)[]; message: string }> }).errors
      ?? [];
...
export function tool<TParams extends ZodType, TOutput>(
  ...
  return {
    ...
    async execute(
      ...
      return definition.execute(validated as Parameters<typeof definition.execute>[0], context);
    },
  };
}
