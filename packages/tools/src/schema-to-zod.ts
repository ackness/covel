...
export function buildFieldsZodFromSchema(
  ...
  for (const attr of schema.attributes) {
    shape[attr.id] = attributeToZod(attr).optional().describe(attrDescribe(attr));
  }
  ...
  switch (attr.type) {
    ...
    case 'object': {
      ...
      for (const sub of attr.subSchema) {
        innerShape[sub.id] = attributeToZod(sub).optional().describe(attrDescribe(sub));
      }
