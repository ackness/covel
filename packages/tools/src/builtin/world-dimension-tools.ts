...
function isLocaleMap(value: unknown): value is Record<string, string> {
  ...
  return entries.length > 0 &&
    entries.every(([key, item]) => localeKeyRe.test(key) && typeof item === 'string');
}
...
function resolveI18nValue(value: unknown, locale?: string): unknown {
  ...
  if (isLocaleMap(value)) {
    ...
    if (languageOnly) {
      const exact = Object.entries(value).find(([key]) => key.split('-')[0] === languageOnly);
...
function getByPath(value: unknown, path: string): { found: boolean; value?: unknown; error?: string } {
  ...
  return tool({
    ...
    parameters: z.object({
      queries: z.array(z.object({
        dimension: z.enum(dimensionKeys).describe('世界维度键，例如 geography、factions、powerSystem、tone'),
        path: z.string().min(1).optional().describe('可选字段路径，例如 "regions[0].name"、"contentRating"'),
      })).min(1).max(20).describe('要查询的维度字段列表'),
      resolveI18n: z.boolean().default(true).describe('是否将 i18n 文本对象按 session locale 解析为字符串，默认 true'),
    }),
    execute: async (params, context) => {
      ...
      async function loadDimension(dimension: DimensionKey): Promise<LoadedDimension | null> {
        ...
        if (providerPluginId) {
          ...
          if (record) {
            const loaded = { source: 'plugin-data' as const, value: record.value };
        ...
        const metadata = isPlainObject(world?.metadata) ? world.metadata : undefined;
        const dimensions = isPlainObject(metadata?.dimensions) ? metadata.dimensions : undefined;
        if (dimensions && dimension in dimensions) {
          const loaded = { source: 'world-metadata' as const, value: dimensions[dimension] };
          dimensionCache.set(dimension, loaded);
          return loaded;
        }
      ...
      for (const query of params.queries) {
        ...
        if (!loaded) {
          results.push({
            ...
            source: null,
            value: null,
            error: 'Dimension not found in plugin_data.entries or world metadata',
            label,
          });
          continue;
        }

        const raw = query.path ? getByPath(loaded.value, query.path) : { found: true, value: loaded.value };
        if (!raw.found) {
          results.push({
            dimension: query.dimension,
            path: query.path,
        ...
        const value = params.resolveI18n ? resolveI18nValue(raw.value, locale) : raw.value;
        results.push({
          dimension: query.dimension,
          path: query.path,
          found: true,
