...
interface RecallSearcher {
  search(sessionId: string, query: string, limit?: number): Promise<readonly {
    turnId: string; role: string; content: string; score: number; timestamp: string;
  }[]>;
}
...
interface ArchivalSearcher {
  search(sessionId: string, query: string, limit?: number): Promise<readonly {
    key: string; content: string; score: number; source: string; pluginId?: string;
  }[]>;
}

/** Minimal interface for core memory block access. */
interface MemoryBlockReader {
  getBlock(sessionId: string, label: string): Promise<{ label: string; content: string; updatedAt: string } | null>;
...
export function createMemoryTools(deps: MemoryToolDeps): ToolModule[] {
  ...
  tools.push(tool({
    name: 'memory-search',
    description: '搜索记忆。可搜索对话历史（recall）和长期知识库（archival，包括 codex 条目、lorebook、角色信息）。当你需要回忆之前发生的事件或查找世界设定时使用。',
    parameters: z.object({
      query: z.string().min(1).describe('搜索关键词或自然语言查询'),
      scope: z.enum(['recall', 'archival', 'all']).optional()
        .describe('搜索范围：recall=对话历史, archival=知识库, all=全部（默认）'),
    ...
    execute: async (params, context) => {
      ...
      if (scope === 'recall' || scope === 'all') {
        const recallResults = await deps.recall.search(context.sessionId, params.query, limit);
      ...
      if (scope === 'archival' || scope === 'all') {
        const archivalResults = await deps.archival.search(context.sessionId, params.query, limit);
  ...
  tools.push(tool({
    ...
    description: '读取一个核心记忆块的内容。可用的块：story_state（剧情状态）、scene（当前场景）、character_relationships（角色关系）、player_profile（玩家状态）。',
    parameters: z.object({
      label: z.enum(['story_state', 'scene', 'character_relationships', 'player_profile'])
        .describe('记忆块标签'),
    }),
    execute: async (params, context) => {
      const block = await deps.blocks.getBlock(context.sessionId, params.label);
      if (!block) {
  ...
  tools.push(tool({
    ...
    description: '更新一个核心记忆块。使用此工具记录重要的剧情进展、场景变化或角色关系变化。内容应是完整的替换文本（不是增量），300-500 字以内。',
    parameters: z.object({
      label: z.enum(['story_state', 'scene', 'character_relationships', 'player_profile'])
        .describe('要更新的记忆块标签'),
    ...
    execute: async (params, context) => {
      await deps.blocks.updateBlock(context.sessionId, params.label, params.content);
      return {
        updated: true,
        label: params.label,
