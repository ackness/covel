...
import { buildSessionCharacterWriteTools, buildFieldsZodFromSchema } from '../src/index.js';
...
const sampleSchema: CharacterAttributeSchema = {
  ...
  attributes: [
    ...
    { id: 'powerTier', name: '境界', type: 'enum', options: ['练气', '筑基', '金丹'], category: 'stats' },
    ...
    { id: 'relationships', name: '人际', type: 'map', valueType: 'string', category: 'social' },
  ],
};
...
describe('buildFieldsZodFromSchema', () => {
  ...
  it('produces a Zod object whose JSON schema lists each declared attribute', () => {
    ...
    expect(Object.keys(props).sort()).toEqual([
      'awakened', 'equipment', 'hp', 'powerTier', 'relationships', 'skills',
    ]);
  });
  ...
  it('preserves number bounds, enum options, array item type, and nested object shape', () => {
    ...
    expect(props.hp).toMatchObject({ type: 'number', minimum: 0, maximum: 100 });
    expect(props.powerTier).toMatchObject({ type: 'string', enum: ['练气', '筑基', '金丹'] });
    expect((props.skills as { type: string; items: { type: string } }).items.type).toBe('string');
    expect(props.awakened.type).toBe('boolean');

    // Nested object: the equipment schema should expose `weapon` and `consumables`
    const equipment = props.equipment as { type: string; properties: Record<string, unknown> };
    expect(equipment.type).toBe('object');
    expect(Object.keys(equipment.properties).sort()).toEqual(['consumables', 'weapon']);
...
describe('buildSessionCharacterWriteTools', () => {
  it('emits a create-character tool whose jsonSchema reflects the session schema', () => {
    const [createTool] = buildSessionCharacterWriteTools(stubStore, {}, sampleSchema);
    ...
    expect(Object.keys(props).sort()).toEqual(['description', 'fields', 'name', 'type']);

    // The fields property is an object with named keys per attribute
    const fields = props.fields as { type: string; properties: Record<string, unknown> };
  ...
  it('emits an update-character tool with the same schema-aware fields shape', () => {
    const [, updateTool] = buildSessionCharacterWriteTools(stubStore, {}, sampleSchema);
    ...
    expect(fields.properties.hp).toMatchObject({ type: 'number', minimum: 0, maximum: 100 });
  });
});
