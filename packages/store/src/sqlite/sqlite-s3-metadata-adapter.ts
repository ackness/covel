 ...
 */
export function createSqliteS3MetadataAdapter(dbPath: string): S3MediaMetadataAdapter {
  ...
  return {
    ...
    async getAsset(id) {
      const row = selectAsset.get(id) as Parameters<typeof rowToRecord>[0] | undefined;
    ...
    async listAssets() {
      const rows = selectAllAssets.all() as Array<Parameters<typeof rowToRecord>[0]>;
