  ...
  const store: DataStore & VectorStoreCapability & VectorModelOps = {
    ...
    async deleteSession(id) {
      ...
      for (const [k, v] of stateEntries) {
        if (v.sessionId === id) stateEntries.delete(k);
      }
      for (const [k, v] of characters) {
        if (v.sessionId === id) characters.delete(k);
      }
      for (const [k, v] of pluginData) {
        if (v.sessionId === id) pluginData.delete(k);
      }
      for (const [k, v] of pluginConfigs) {
        if (v.sessionId === id) pluginConfigs.delete(k);
      }
      for (const [k, v] of snapshots) {
        if (v.sessionId === id) snapshots.delete(k);
      }
      for (const [k, v] of suspensions) {
        if (v.sessionId === id) suspensions.delete(k);
      }
      ...
      for (const [k, v] of vectorRows) {
        if (v.sessionId === id) vectorRows.delete(k);
      }
    ...
    async getRuntimeOutput(sessionId, id) {
      return (
        runtimeOutputs.find((r) => r.sessionId === sessionId && r.id === id) ??
        null
      );
    },
    ...
    async getStateEntry(sessionId, tableName, fieldName) {
      return (
        stateEntries.get(stateEntryKey(sessionId, tableName, fieldName)) ?? null
      );
    },
    ...
    async getPluginData(sessionId, pluginId, namespace, key) {
      return (
        pluginData.get(pluginDataKey(sessionId, pluginId, namespace, key)) ??
        null
      );
    },
    ...
    async searchVectors(
      ...
      for (const row of vectorRows.values()) {
        ...
        if (input.pluginId !== undefined && row.pluginId !== input.pluginId)
          continue;
        if (input.namespace !== undefined && row.namespace !== input.namespace)
          continue;
