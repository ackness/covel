  ...
  describe(`DataStore Contract: ${name}`, () => {
    ...
    describe("PluginData", () => {
      ...
      it("listPluginDataSessionScope returns all pluginIds and namespaces for a session", async () => {
        ...
        expect(pluginIds).toEqual(
          new Set(["plugin-a", "plugin-b", "plugin-c"]),
        );
    ...
    describe("RuntimeOutputs", () => {
      ...
      it("should filter by sessionId", async () => {
        await store.saveRuntimeOutput(
          makeRuntimeOutput({ sessionId: "sess-1" }),
        );
        await store.saveRuntimeOutput(
          makeRuntimeOutput({ sessionId: "sess-2" }),
        );
    ...
    describe("LorebookEntries (S3-T2)", () => {
      it("returns an empty list when the session has no entries", async () => {
        const result =
          await store.listSessionLorebookEntries("sess-lore-empty");
    ...
    describe("deleteSession cascade", () => {
      it("removes all session-scoped rows across every collection", async () => {
        ...
        expect(await store.listStateEntries(sessionId, "stats")).toHaveLength(
          0,
        );
        expect(
        ...
        expect(await store.listPluginData(sessionId, "plugin-1")).toHaveLength(
          0,
        );
