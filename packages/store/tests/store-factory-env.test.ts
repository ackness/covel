...
describe('store factory env wiring', () => {
  ...
  it('resolves invalid STORE_BACKEND through the shared env fallback', () => {
    withEnv({ STORE_BACKEND: 'postgres', SQLITE_PATH: undefined, DATABASE_URL: undefined });

    expect(resolveBackendFromEnv()).toBe('sqlite');
  });

  it('creates an in-memory store when STORE_BACKEND=memory', async () => {
    withEnv({ STORE_BACKEND: 'memory', SQLITE_PATH: undefined, DATABASE_URL: undefined });
  ...
  it('fails fast for pg backend when DATABASE_URL is missing', async () => {
    withEnv({ STORE_BACKEND: 'pg', SQLITE_PATH: undefined, DATABASE_URL: undefined });

    await expect(createStoreFromEnv()).rejects.toThrow('DATABASE_URL required for pg backend');
  });
});
