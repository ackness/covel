...
describe('WriteCollector', () => {
  ...
  describe('commit: no conflicts', () => {
    it('should commit all writes when targeting different fields', () => {
      wc.recordWrite(makeWrite({ field: 'hp', newValue: 80, pluginId: 'a', runtimeId: 'ra' }));
      wc.recordWrite(makeWrite({ field: 'mp', newValue: 30, pluginId: 'b', runtimeId: 'rb' }));
  ...
  describe('commit: conflict detected', () => {
    it('should detect conflict when two writes target same field with different values', () => {
      wc.recordWrite(makeWrite({ field: 'hp', newValue: 80, pluginId: 'a', runtimeId: 'ra', priority: 500 }));
      wc.recordWrite(makeWrite({ field: 'hp', newValue: 60, pluginId: 'b', runtimeId: 'rb', priority: 400 }));
  ...
  describe('commit: same value = no conflict', () => {
    it('should merge writes with identical values without conflict', () => {
      wc.recordWrite(makeWrite({ field: 'hp', newValue: 80, pluginId: 'a', runtimeId: 'ra' }));
      wc.recordWrite(makeWrite({ field: 'hp', newValue: 80, pluginId: 'b', runtimeId: 'rb' }));
  ...
  describe('commit result structure', () => {
    it('should have correct committed and conflicts arrays', () => {
      wc.recordWrite(makeWrite({ table: 'stats', field: 'hp', newValue: 80, pluginId: 'a', runtimeId: 'ra' }));
      wc.recordWrite(makeWrite({ table: 'stats', field: 'hp', newValue: 60, pluginId: 'b', runtimeId: 'rb' }));
      wc.recordWrite(makeWrite({ table: 'stats', field: 'mp', newValue: 30, pluginId: 'c', runtimeId: 'rc' }));
