...
export type UIPartStatus = 'pending' | 'streaming' | 'success' | 'error' | 'paused';
...
export type UIRenderInstruction = UIRenderPartsInstruction | UIRenderLegacyInstruction;

export function isUIRenderPartsInstruction(value: unknown): value is UIRenderPartsInstruction {
  return !!value
    && typeof value === 'object'
...
export function normalizeUIRenderInstruction(
  ...
  if (isUIRenderPartsInstruction(instruction)) {
    const parts = instruction.parts.map((rawPart, index) => {
      ...
      return {
        id: typeof part.id === 'string' && part.id.length > 0
          ? part.id
          : `${fallbackId}-part-${index + 1}`,
        type: typeof part.type === 'string' && part.type.length > 0
          ? part.type
          : 'card',
    ...
    return {
      ...
      status: normalizePartStatus(instruction.status ?? inferInstructionStatus(parts)),
    };
  }
  ...
  const part: UIRenderPart = {
    id,
    type: typeof legacy.type === 'string' && legacy.type.length > 0 ? legacy.type : 'card',
    status: 'success',
    content,
  };
  return {
