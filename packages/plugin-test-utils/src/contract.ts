...
import type { AssetGeneratePayload, RuntimeResult, TurnResult } from '@covel/shared';
...
export function expectAssetGenerated(
  ...
  const options = typeof expected === 'string' ? { modality: expected } : expected;
  ...
  const validAssets = candidates
    ...
    .filter((entry): entry is { candidate: AssetCandidate; parsed: AssetGeneratePayload } => entry.parsed !== null);

  if (validAssets.length === 0) {
    throw new Error(`Expected asset.generate payload with MediaRef shape; checked ${formatPaths(candidates)}`);
  }
  ...
  const matching = validAssets.find((entry) => entry.parsed.modality === options.modality);
  if (matching) {
    return matching.parsed;
  }

  const actual = validAssets.map((entry) => entry.parsed.modality).join(', ');
  throw new Error(`Expected asset.generate modality "${options.modality}", received ${actual}`);
}

function collectAssetCandidates(source: AssertionSource): AssetCandidate[] {
  if (isRuntimeResultArray(source)) {
    return source.flatMap((result, index) => collectFromRuntimeResult(result, `runtimeResults[${index}]`));
  }

  if (isTurnResult(source)) {
    return source.runtimeResults.flatMap((result, index) => collectFromRuntimeResult(result, `runtimeResults[${index}]`));
  }
...
function collectFromRuntimeResult(result: RuntimeResult, path: string): AssetCandidate[] {
  if (!result.output) {
    return [];
  }
...
function isRuntimeResultArray(value: unknown): value is readonly RuntimeResult[] {
...
function isRuntimeResult(value: unknown): value is RuntimeResult {
  return isRecord(value) && 'runtimeId' in value && 'pluginId' in value && 'output' in value;
}
