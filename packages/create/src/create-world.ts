...
function log(options: CreateWorldOptions, level: 'info' | 'warn' | 'error', ...args: unknown[]): void {
...
export async function createWorld(options: CreateWorldOptions): Promise<CreateResult> {
  ...
  log(options, 'info', 'start concept="', options.concept, '" locale=', locale, 'model=', options.model ?? 'default');

  let lastErrors: string[] = [];

  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    log(options, 'info', `attempt ${attempt + 1}/${MAX_RETRIES + 1}: sending LLM request`);
    ...
    log(options, 'info', `LLM responded in ${Date.now() - llmStart}ms contentLength=${response.content?.length ?? 0}`);
    ...
    if (!parsed) {
      ...
      log(options, 'warn', 'attempt', attempt + 1, 'parseWorldOutput failed (delimiters missing)');
      continue;
    }

    log(options, 'info', 'parsed OK: yamlLength=', parsed.yaml.length, 'loreLength=', parsed.lore.length);
    ...
    try {
      ...
      log(options, 'warn', 'attempt', attempt + 1, 'YAML parse failed:', lastErrors[0]);
      continue;
    }
    ...
    if (!validation.valid) {
      ...
      log(options, 'warn', 'attempt', attempt + 1, 'validation failed with', lastErrors.length, 'errors');
      continue;
    }
