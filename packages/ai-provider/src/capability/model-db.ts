...
export interface ModelDatabase {
  ...
  search(query: string, limit?: number): Array<{ id: string; entry: ModelDbEntry }>;
...
export function createModelDatabase(
  ...
  function toCapability(entry: ModelDbEntry): ModelCapability {
    const pricing: ModelPricing | undefined =
      (entry.inputPerMToken ?? entry.outputPerMToken ?? entry.perImage ?? entry.audioInputPerMToken ?? entry.audioOutputPerMToken)
        ? {
            ...(entry.inputPerMToken != null ? { inputPerMToken: entry.inputPerMToken } : {}),
            ...(entry.outputPerMToken != null ? { outputPerMToken: entry.outputPerMToken } : {}),
            ...(entry.perImage != null ? { perImage: entry.perImage } : {}),
            ...(entry.audioInputPerMToken != null ? { audioInputPerMToken: entry.audioInputPerMToken } : {}),
            ...(entry.audioOutputPerMToken != null ? { audioOutputPerMToken: entry.audioOutputPerMToken } : {}),
          }
        : undefined;
  ...
  function search(query: string, limit = 20): Array<{ id: string; entry: ModelDbEntry }> {
  ...
  return {
    getInfo: () => ({ updatedAt: data.updatedAt, count: data.count, source: data.source }),
    lookup,
    toCapability,
    search,
    listAll: () => data.models,
...
function transformLiteLlmEntry(raw: Record<string, unknown>): ModelDbEntry | null {
  ...
  const entry: ModelDbEntry = {
    ...
    output,
    features,
    contextWindow: (raw.max_input_tokens as number) ?? (raw.max_tokens as number) ?? 0,
    maxOutputTokens: (raw.max_output_tokens as number) ?? (raw.max_tokens as number) ?? 0,
    mode,
  };

  if (raw.litellm_provider) entry.litellmProvider = raw.litellm_provider as string;
  if (raw.input_cost_per_token) entry.inputPerMToken = roundPrice((raw.input_cost_per_token as number) * 1e6);
  if (raw.output_cost_per_token) entry.outputPerMToken = roundPrice((raw.output_cost_per_token as number) * 1e6);
  if (raw.input_cost_per_image) entry.perImage = raw.input_cost_per_image as number;
  if (raw.input_cost_per_audio_token) entry.audioInputPerMToken = roundPrice((raw.input_cost_per_audio_token as number) * 1e6);
  if (raw.output_cost_per_audio_token) entry.audioOutputPerMToken = roundPrice((raw.output_cost_per_audio_token as number) * 1e6);

  return entry;
}

function deriveInput(raw: Record<string, unknown>, mode: string): InputModality[] {
  const explicit = raw.supported_modalities as string[] | undefined;
  if (explicit?.length) return explicit.filter((s): s is InputModality => ["text", "image", "audio", "video", "file"].includes(s));

  const m: InputModality[] = [];
  switch (mode) {
    case "chat": case "completion": case "responses":
      m.push("text");
      if (raw.supports_vision) m.push("image");
      if (raw.supports_audio_input) m.push("audio");
      if (raw.supports_pdf_input) m.push("file");
      break;
    case "audio_transcription": m.push("audio"); break;
    case "audio_speech": case "image_generation": case "video_generation": m.push("text"); break;
    case "image_edit": m.push("text", "image"); break;
    case "embedding": m.push("text"); break;
    case "ocr": m.push("image"); break;
    case "realtime": m.push("text", "audio"); break;
    default: m.push("text");
  }
  return m;
}

function deriveOutput(raw: Record<string, unknown>, mode: string): OutputModality[] {
  const explicit = raw.supported_output_modalities as string[] | undefined;
  if (explicit?.length) return explicit.filter((s): s is OutputModality => ["text", "image", "audio", "embedding"].includes(s));
  ...
  switch (mode) {
    ...
    case "audio_transcription": case "ocr": return ["text"];
    case "audio_speech": case "realtime": return ["audio"];
    case "image_generation": case "image_edit": return ["image"];
