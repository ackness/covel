...
function transformEntry(key: string, raw: LiteLlmEntry): ModelDbEntry | null {
  ...
  if (raw.input_cost_per_audio_token) {
    entry.audioInputPerMToken = roundPrice(raw.input_cost_per_audio_token * 1_000_000);
  }
  if (raw.output_cost_per_audio_token) {
    entry.audioOutputPerMToken = roundPrice(raw.output_cost_per_audio_token * 1_000_000);
  }

  return entry;
}

function deriveInputModalities(raw: LiteLlmEntry, mode: string): InputModality[] {
...
function deriveOutputModalities(raw: LiteLlmEntry, mode: string): OutputModality[] {
  // Use explicit arrays if available
  if (raw.supported_output_modalities && raw.supported_output_modalities.length > 0) {
