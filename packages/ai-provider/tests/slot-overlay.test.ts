...
function makeDeps() {
  const providerRegistry = createProviderRegistry({
    providerDefaults: {
      deepseek: { baseUrl: "https://api.deepseek.com", protocol: "openai-chat-v1" },
    },
  });
...
describe("applySlotOverlay", () => {
  ...
  it("is a no-op when overrides are undefined or empty", () => {
    ...
    expect(applySlotOverlay(deps, { customPresets: [] })).toBeInstanceOf(Function);
    // Still works after the no-op cleanup — doesn't touch registries.
    expect(deps.presetRegistry.listPresets().map((p) => p.id)).toEqual(["ds-chat"]);
  });
  ...
  it("degrades to no-op when the registry surface lacks overlay methods", () => {
    ...
    const cleanup = applySlotOverlay(stubDeps, {
      customPresets: [
        { id: "x", name: "X", provider: "p", model: "m", baseUrl: "https://x.example" },
      ],
    });
