...
describe('buildContextAsync — runtime inject regression', () => {
  it('resolves runtime injects identically to the sync path', async () => {
    ...
    expect(asyncResult.systemPrompt).toContain('<narrator-output>你走到了山脚下。</narrator-output>');
  });
...
describe('buildContextAsync — plugin-data inject', () => {
  function makeParams(
    store: DataStore,
    extraInjects: RuntimeManifest['input'] extends { inject?: infer U } ? U : never = [],
  ): ContextBuildParams {
    const manifest = makeManifest({
      input: {
        inject: [
  ...
  it('injects <existing-entries>暂无</existing-entries> when namespace is empty', async () => {
    ...
    expect(result.systemPrompt).toContain('<existing-entries>暂无</existing-entries>');
  });
  ...
  it('renders summary format with key + updatedAt + truncated value', async () => {
    ...
    const result = await buildContextAsync(makeParams(store));
    expect(result.systemPrompt).toContain('- codex-fire-mountain | 2025-01-02T00:00:00.000Z');
    expect(result.systemPrompt).toContain('- codex-blue-river | 2025-01-04T00:00:00.000Z');
  ...
  it('propagates listPluginData errors (no silent fallback)', async () => {
    ...
    await expect(buildContextAsync(makeParams(store))).rejects.toThrow('store offline');
  });
  ...
  it('throws when store is missing but plugin-data inject present', async () => {
    ...
    void _store;
    await expect(buildContextAsync(withoutStore as ContextBuildParams)).rejects.toThrow(
      /store is required/i,
    );
  });
});
...
describe('buildContextAsync — two-pass truncation', () => {
  ...
  it('returns all entries when count <= maxEntries (no truncation)', async () => {
    const entries = [
      makeEntry('codex-a', { n: 1 }, '2025-01-01T00:00:00.000Z', '2025-01-01T00:00:00.000Z'),
      makeEntry('codex-b', { n: 2 }, '2025-01-02T00:00:00.000Z', '2025-01-02T00:00:00.000Z'),
    ];
    const store = makeStoreStub(entries);
    const manifest = makeManifest({
      input: {
...
describe('buildContextAsync — format variants', () => {
  ...
  it('full outputs the complete JSON value', async () => {
    ...
    expect(result.systemPrompt).toContain('- codex-test: {"title":"测试","content":"short"}');
  });

  it('summary includes updatedAt and compact JSON', async () => {
    const result = await buildWithFormat('summary');
    expect(result.systemPrompt).toContain('- codex-test | 2025-01-02T00:00:00.000Z |');
