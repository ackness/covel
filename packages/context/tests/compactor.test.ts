...
function makeMinimalStore(): DataStore {
  ...
  return {
    saveSessionSummary: vi.fn(async (s: SessionSummaryRecord) => { summaries.push(s); }),
    listSessionSummaries: vi.fn(async () => [...summaries]),
    deleteSessionSummaries: vi.fn(async () => {}),
    tagTurnMessagesCompacted: vi.fn(async (
      _sessionId: string,
...
function makeFastLlm(response = 'Compact summary content.'): CompactorLLMAdapter {
  return {
...
describe('maybeCompact', () => {
  ...
  describe('under threshold — no compaction', () => {
    it('returns { compacted: false } when token count is below threshold', async () => {
      ...
      const result = await maybeCompact('sess-1', 'system prompt', messages, deps);
  ...
  describe('over threshold — compaction triggered', () => {
    ...
    it('stores the summary with correct sessionId and focusSections', async () => {
      ...
      const saved = (store.saveSessionSummary as ReturnType<typeof vi.fn>).mock.calls[0][0] as SessionSummaryRecord;
  ...
  describe('protection rules', () => {
    it('does not compact when protect window covers all messages', async () => {
      ...
      const deps: CompactorDeps = { store, estimator, fastSlotLlm, contextWindow: 100 };
    ...
    it('protects at least the specified number of user turns', async () => {
      ...
      for (let i = 0; i < 12; i++) {
        messages.push(makeTurnMessage(`m${i}`, i % 2 === 0 ? 'user' : 'assistant', `content ${i} `.repeat(80)));
      }
      const deps: CompactorDeps = { store, estimator, fastSlotLlm, contextWindow: 500 };
      ...
      const taggedIds = (store.tagTurnMessagesCompacted as ReturnType<typeof vi.fn>).mock.calls[0][1] as string[];

      // Count user messages in tagged set
      const taggedSet = new Set(taggedIds);
      const lastTwoUserTurns = messages.filter((m) => m.role === 'user').slice(-2);
  ...
  describe('no cascade compaction', () => {
    it('skips compaction if any to-compact message is already tagged', async () => {
      ...
      (messages[0] as { compactedAtTurnId?: string }).compactedAtTurnId = 'existing-summary';
  ...
  describe('LLM failure handling', () => {
    it('returns { compacted: false } and warns when fast LLM throws', async () => {
      ...
      const messages = makeSimpleHistory(20);
      const deps: CompactorDeps = { store, estimator, fastSlotLlm: failingLlm, contextWindow: 1_000 };

      const result = await maybeCompact('sess-1', '', messages, deps);

      expect(result.compacted).toBe(false);
      expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('LLM unavailable'));
      warnSpy.mockRestore();
    });
  });
