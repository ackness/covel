...
export async function maybeCompact(
  ...
  const protectLastNUserTurns = opts?.protectLastNUserTurns ?? DEFAULT_PROTECT_LAST_USER_TURNS;
  const protectLastNMessages = opts?.protectLastNMessages ?? DEFAULT_PROTECT_LAST_N_MESSAGES;
  const focusSections = opts?.focusSections ?? [];
  const locale = opts?.locale ?? 'zh-CN';

  // 1. Estimate total tokens
  const systemTokens = deps.estimator(systemPrompt);
  const messageTokens = messages.reduce((sum, m) => sum + deps.estimator(m.content), 0);
  ...
  const protectStart = computeProtectStart(messages, protectLastNUserTurns, protectLastNMessages);
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    console.warn(`[compactor] Fast LLM call failed for session ${sessionId}: ${message}. Skipping compaction.`);
    return { compacted: false };
  }
  ...
  try {
    await deps.store.addTraceEvent({
      ...
      payload: {
        ...
        tokenSavings: toCompact.reduce((sum, m) => sum + deps.estimator(m.content), 0),
        focusSections,
      },
      createdAt: now,
    });
