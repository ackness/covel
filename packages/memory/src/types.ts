...
export interface MemoryManager {
  ...
  getBlock(sessionId: string, label: CoreMemoryLabel): Promise<CoreMemoryBlock | null>;

  /** Write (create or overwrite) a single block. */
  updateBlock(sessionId: string, label: CoreMemoryLabel, content: string): Promise<void>;

  /** Batch-write multiple blocks atomically. */
  updateBlocks(sessionId: string, updates: ReadonlyMap<CoreMemoryLabel, string>): Promise<void>;
...
export interface RecallSearcher {
  search(sessionId: string, query: string, limit?: number): Promise<readonly RecallSearchResult[]>;
}
...
export interface ArchivalSearcher {
  search(sessionId: string, query: string, limit?: number): Promise<readonly ArchivalSearchResult[]>;
}
...
export interface MemorySystem {
  ...
  compact(params: {
    ...
    messages: readonly { id: string; role: string; content: string; turnId?: string; createdAt: string }[];
