...
export function createCompactor(
  ...
  const protectUserTurns = config?.protectLastNUserTurns ?? DEFAULT_PROTECT_USER_TURNS;
    ...
    async compact(params: {
      ...
      try {
        ...
        const response = await deps.llm.complete({
          systemPrompt: lang === 'zh' ? COMPACTION_SYSTEM_PROMPT_ZH : COMPACTION_SYSTEM_PROMPT_EN,
        ...
        await deps.store.saveSessionSummary({
          ...
          turnRangeEnd: toCompact[toCompact.length - 1].turnId ?? toCompact[toCompact.length - 1].id,
          content: summary,
          focusSections: [],
          createdAt: now,
        });
        ...
        await deps.store.tagTurnMessagesCompacted(sessionId, compactedIds, summaryId);
