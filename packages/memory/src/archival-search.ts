 ...
 */
export function createKeywordArchivalSearcher(store: DataStore): ArchivalSearcher {
  return {
    async search(sessionId, query, limit = 10): Promise<readonly ArchivalSearchResult[]> {
      ...
      try {
        ...
        for (const char of characters) {
          ...
          if (score > 0) {
            results.push({
              key: char.name,
              content: `[${char.type}] ${char.name}: ${char.description ?? ''}`.slice(0, 500),
              score,
              source: 'character',
            });
          }
...
function scoreText(text: string, queryLower: string, queryTerms: string[]): number {
