...
function createMockStore() {
  ...
  const makePdKey = (sid: string, pid: string, ns: string, key: string) => `${sid}:${pid}:${ns}:${key}`;
  ...
  return {
    ...
    async setPluginData(record: PluginDataRow) {
      pluginData.set(makePdKey(record.sessionId, record.pluginId, record.namespace, record.key), record);
    },
  };
}
...
describe('CoreMemoryManager', () => {
  ...
  describe('plugin-data mirror', () => {
    ...
    beforeEach(() => {
      ...
      mirrorManager = createMemoryManager(mirrorStore as any, { pluginId: PLUGIN_ID });
    });
    ...
    it('should mirror updateBlock to plugin-data with rich payload shape', async () => {
      ...
      const row = mirrorStore.pluginData.get(`sess-1:${PLUGIN_ID}:blocks:scene`);
      ...
      expect(value.displayName).toEqual(CORE_MEMORY_LABEL_INFO.scene.displayName);
    ...
    it('should mirror updateBlocks (batch) to plugin-data with rich payload shape', async () => {
      ...
      const story = mirrorStore.pluginData.get(`sess-1:${PLUGIN_ID}:blocks:story_state`);
      const player = mirrorStore.pluginData.get(`sess-1:${PLUGIN_ID}:blocks:player_profile`);
      ...
      expect(storyValue.displayName).toEqual(CORE_MEMORY_LABEL_INFO.story_state.displayName);
      ...
      expect(playerValue.displayName).toEqual(CORE_MEMORY_LABEL_INFO.player_profile.displayName);
    ...
    it('should reflect truncation in mirrored charCount and content', async () => {
      ...
      const row = mirrorStore.pluginData.get(`sess-1:${PLUGIN_ID}:blocks:story_state`);
      const value = row!.value as Record<string, unknown>;
    ...
    it('should fall back to label as displayName for unknown labels', async () => {
      ...
      await mirrorManager.updateBlock('sess-1', 'unknown_label' as any, 'content');

      const row = mirrorStore.pluginData.get(`sess-1:${PLUGIN_ID}:blocks:unknown_label`);
      expect(row).toBeDefined();
      const value = row!.value as Record<string, unknown>;
      expect(value.displayName).toEqual({ zh: 'unknown_label', en: 'unknown_label' });
