...
export type RuntimeExecuteFn = (manifest: RuntimeManifest) => Promise<RuntimeResult>;

export interface FailureResolution {
  /** Runtimes to skip (dependent on failed runtime). */
  readonly skip: ReadonlyArray<{ readonly manifest: RuntimeManifest; readonly reason: string }>;
  /** Runtimes safe to continue. */
  readonly continue: ReadonlyArray<{ readonly manifest: RuntimeManifest; readonly degraded: boolean }>;
}
...
export function resolveFailure(
  ...
  const skip: Array<{ readonly manifest: RuntimeManifest; readonly reason: string }> = [];
  const cont: Array<{ readonly manifest: RuntimeManifest; readonly degraded: boolean }> = [];
