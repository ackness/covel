...
export async function executeTurn(
	...
	if (deps.store) {
		...
		if (isEnvEnabled("COVEL_SNAPSHOTS_V1")) {
			try {
				const { buildSnapshotPayload } = await import(
					"./snapshot-payload-builder.js"
				);
...
async function executeOneRuntime(
	...
	try {
		...
		if (loaded.guard) {
			...
			const guardManualPayload =
				input.manualTrigger?.runtimeId === manifest.name
					? input.manualTrigger.payload
					: undefined;
			const guardUserSettings = resolveUserSettings(
				manifest,
				input.userSettings,
			);
		...
		const hasBuiltinTools =
			manifest.tools?.builtin !== undefined &&
			manifest.tools.builtin.length > 0;
		const hasLocalTools =
		...
		while (steps < effectiveMaxSteps && Date.now() < deadline) {
			...
			if (response.toolCalls.length > 0) {
				...
				if (retryPolicy.loopDetectionThreshold > 0) {
					...
					if (
						detectToolLoop(identityCalls, retryPolicy.loopDetectionThreshold)
					) {
		...
		const structuredToolOutput =
			findLastStructuredToolOutput(executedToolCalls);
		if (finalContent) {
			const parsed = parseFinalOutputEnvelope(finalContent);
		...
		if (
			...
			output.narrativeOutput = sanitizeStoryNarrativeText(
				output.narrativeOutput,
			);
		}
