 ...
 *
 * @param input - Player's turn input
 * @param activeRuntimes - All active RuntimeManifests for this session (sorted by priority)
    ...
    for (const msg of thisTurnMessages) {
      if (msg.pendingInput && msg.sourcePluginId) {
        pendingInputs.push({
          ...
          form: msg.pendingInput as Record<string, unknown>,
...
async function executeOneRuntime(
  ...
  try {
    ...
    // Extract form from create-form tool call (if any)
    const formToolCall = executedToolCalls.find(tc => tc.name === 'create-form');
    if (formToolCall?.success) {
      try {
        const formArgs = JSON.parse(formToolCall.arguments) as Record<string, unknown>;
        // Override output with tool-call-derived data
        output.narrativeTemplate = formArgs.narrativeTemplate;
        output.form = {
          formId: formArgs.formId,
          title: formArgs.title,
          fields: formArgs.fields,
          submitLabel: formArgs.submitLabel,
        };
        // If LLM produced narrative text alongside the tool call, use it
        if (finalContent && !output.narrativeOutput) {
          output.narrativeOutput = finalContent;
        }
      } catch {
        // Failed to parse tool arguments — leave output as-is
      }
    }
    ...
    if (deps.store) {
      ...
      // Extract pendingInput form if present
      const form = output.form as Record<string, unknown> | undefined;
      const pendingInput = form?.formId ? form : undefined;
