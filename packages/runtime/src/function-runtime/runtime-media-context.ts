...
export function createRuntimeMediaContext(
  ...
  return {
    ...
    async ingestUrl(url, opts) {
      ...
      const ref = await ingestUrlIntoStore(
        mediaStore,
        utils,
        url,
        opts,
...
async function ingestUrlIntoStore(
  ...
  const maxBytes = positiveInteger(
    opts?.maxBytes,
    contextOptions.defaultMaxBytes ?? DEFAULT_MAX_BYTES,
  );
  const timeoutMs = positiveInteger(
    opts?.timeoutMs,
    contextOptions.defaultTimeoutMs ?? DEFAULT_TIMEOUT_MS,
  );
...
function normalizeAndValidateUrl(
  ...
  if (!verdict.ok) {
    throw new Error(
      `media ingest URL rejected: ${verdict.reason ?? parsed.toString()}`,
    );
  }
  return parsed;
}
...
async function fetchWithValidatedRedirects(
  ...
  for (
    ...
    currentUrl = normalizeAndValidateUrl(
