...
import type { SessionSnapshot, SnapshotMessage, SnapshotCharacter, SnapshotTraceEvent } from '@covel/shared';
...
export interface SnapshotStore {
  getSession(id: string): Promise<{ id: string; worldId?: string; phase: string; turnCount: number; locale?: string } | null>;
  listMessages(sessionId: string): Promise<readonly { id: string; role: string; content: string; metadata?: unknown; createdAt: string }[]>;
  listCharacters(sessionId: string): Promise<readonly { id: string; name: string; type: string; description?: string; fields?: unknown }[]>;
  listStateSchemas(sessionId: string): Promise<readonly { tableName: string }[]>;
  listStateEntries(sessionId: string, tableName: string): Promise<readonly { tableName: string; fieldName: string; value: unknown }[]>;
  listTraceEvents(sessionId: string): Promise<readonly { type: string; turnId: string; payload: unknown; createdAt: string }[]>;
}
...
export async function buildSessionSnapshot(
  ...
  const [rawMessages, characters, stateSchemas, traceEvents] = await Promise.all([
