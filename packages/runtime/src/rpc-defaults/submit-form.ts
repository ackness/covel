...
export const submitFormHandler: RpcHandler = async (
  ...
  for (const sub of submissions) {
    if (!sub.interactionId || typeof sub.interactionId !== 'string') {
      throw new RpcValidationError('Each submission requires interactionId (string)');
    }
  ...
  const messages = (await store.listTurnMessages(sessionId)) as readonly MessageLike[];
