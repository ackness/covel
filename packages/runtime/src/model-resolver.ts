...
export function createModelResolver(
  ...
  return (manifest: RuntimeManifest, apiOverride?: string): string | undefined => {
    // 1. API override (highest priority — bypasses everything)
    if (apiOverride) {
      // Even API override goes through plugin-local lookup first
      const pluginResolved = resolveFromPluginConfig(manifest.name, apiOverride, pluginLlmConfigs);
    ...
    if (manifest.model) {
      ...
      const pluginResolved = resolveFromPluginConfig(manifest.name, manifest.model, pluginLlmConfigs);
      if (pluginResolved) return pluginResolved;
    ...
    const pluginDefault = resolveFromPluginConfig(manifest.name, 'default', pluginLlmConfigs);
