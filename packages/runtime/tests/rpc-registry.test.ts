...
describe('createRpcHandlerStoreView', () => {
  it('binds action-level RPC reads and writes to the current session and plugin', async () => {
    const store = {
      ...
      savePlayerInput: vi.fn(async (_input: Record<string, unknown>) => undefined),
      setPluginData: vi.fn(async (_record: Record<string, unknown>) => undefined),
      getPluginData: vi.fn(async (sessionId: string, pluginId: string, namespace: string, key: string) => ({ sessionId, pluginId, namespace, key })),
      listPluginData: vi.fn(async (sessionId: string, pluginId: string, namespace?: string) => [{ sessionId, pluginId, namespace }]),
    };
    ...
    await scoped.getPluginData?.('sess-attacker', 'plugin-attacker', 'ns', 'key');
    ...
    expect(store.getPluginData).toHaveBeenCalledWith('sess-real', 'plugin-real', 'ns', 'key');
    expect(store.listPluginData).toHaveBeenCalledWith('sess-real', 'plugin-real', 'ns');
  });
});
