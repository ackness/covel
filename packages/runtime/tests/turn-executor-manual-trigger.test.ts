...
type Handler = (ctx: Record<string, unknown>) => Promise<Record<string, unknown>>;
...
describe('executeTurn: manual trigger', () => {
  it('runs only the targeted runtime and skips unrelated auto runtimes', async () => {
    ...
    const { result, handlerCalls } = await runTurn(
      [target, unrelated],
      {
        ...
        'plug/auto': async () => {
          throw new Error('unrelated auto runtime should not fire during manual trigger');
        },
      },
      {
        sessionId: 'sess-1',
  ...
  it('exposes deps.gateway to function handlers via ctx.gateway', async () => {
    const target = fnManifest('plug/needs-gateway', { trigger: { type: 'manual' } });
  ...
  it('omits ctx.gateway when the executor was constructed without one', async () => {
    const target = fnManifest('plug/gateway-less', { trigger: { type: 'manual' } });
  ...
  it('omits ctx.userSettings entirely when the manifest declares no userSettings', async () => {
    const target = fnManifest('plug/no-settings', { trigger: { type: 'manual' } });
    ...
    expect('userSettings' in (handlerCalls['plug/no-settings']?.[0] ?? {})).toBe(false);
  });
  ...
  it('scopes ctx.userSettings to the runtime\'s own pluginId', async () => {
    ...
    expect(handlerCalls['plug-a/target']?.[0]?.userSettings).toEqual({ theme: 'dark' });
  });

  it('schedules event-triggered followers in the same turn after the manual runtime emits', async () => {
    const target = fnManifest('plug/target', { trigger: { type: 'manual' }, priority: 600 });
  ...
  it('aborts cleanly when the manualTrigger.runtimeId is not in the active set', async () => {
    const unrelated = fnManifest('plug/unrelated', { trigger: { type: 'auto' } });

    const { result, handlerCalls } = await runTurn(
      [unrelated],
      {
        'plug/unrelated': async () => {
          throw new Error('no auto runtime should run when manual target is unknown');
        },
      },
      {
        sessionId: 'sess-1',
