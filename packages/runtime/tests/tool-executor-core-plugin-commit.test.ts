...
describe('ToolExecutor + core plugin pending proposals + commit pipeline', () => {
  it('executes world-init tools, records calls, then commits schema, entries and lorebook rows', async () => {
    ...
    expect(calls.every((call) => call.approvalStatus === 'auto-allowed')).toBe(true);

    const proposals = [
      ...(schemaResult.pendingProposals ?? []),
      ...(entriesResult.pendingProposals ?? []),
    ];
    const commitResults = await createCommitPipeline(store).commitAll(proposals);
    ...
    const entries = await store.listPluginData(context.sessionId, context.pluginId, 'entries');
    expect(entries.map((entry) => entry.key).sort()).toEqual(['factions', 'geography']);
