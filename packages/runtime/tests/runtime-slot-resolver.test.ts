...
describe('resolveRuntimeSlot (PR-6)', () => {
  it('returns session override when runtime ID matches', () => {
    const manifest = makeManifest({ name: 'core-narrator', model: 'fast' });
    const slot = resolveRuntimeSlot(manifest, { 'core-narrator': 'balance' });
  ...
  it('returns undefined when neither session override nor manifest model is set', () => {
    const manifest = makeManifest({ name: 'core-narrator' });
  ...
  it('matches multi-runtime plugin IDs by full runtime name', () => {
    const manifest = makeManifest({
      name: 'core-codex/unlocker',
      pluginId: 'core-codex',
      model: 'fast',
    });
    const slot = resolveRuntimeSlot(manifest, {
      'core-codex/unlocker': 'balance',
      'core-codex/retriever': 'fast',
    });
    expect(slot).toBe('balance');
  });
  ...
  it('does not fan out plugin-level overrides to individual runtimes', () => {
    ...
    // Override key is just "core-codex" — should not match "core-codex/unlocker".
    const slot = resolveRuntimeSlot(manifest, { 'core-codex': 'balance' });
    expect(slot).toBe('fast');
  });

  it('treats empty-string overrides as absent', () => {
    const manifest = makeManifest({ name: 'core-narrator', model: 'fast' });
    const slot = resolveRuntimeSlot(manifest, { 'core-narrator': '' });
    expect(slot).toBe('fast');
  });
