...
describe('core plugin manifest contract', () => {
  it('keeps the Pre-Game chain in framework scheduling order', async () => {
    ...
    expect(schemaGen.tools?.builtin).toEqual(['plugin-data-get', 'plugin-data-list']);
    ...
    const pregamePriorities = [pregame, schemaGen, playerInit].map((manifest) => manifest.priority);
    expect(pregamePriorities).toEqual([10, 40, 50]);
    expect(pregamePriorities.every((priority) => priority >= 0 && priority <= 99)).toBe(true);
  });
  ...
  it('keeps the main-loop DAG around narrator and downstream plugins', async () => {
    ...
    const downstreams = downstreamIds.map((id) => requireRuntime(manifests, id));
  ...
  it('keeps framework-controlled core plugins enabled as core-plugin manifests', async () => {
    ...
    for (const runtimeId of coreRuntimeIds) {
      expect(requireRuntime(manifests, runtimeId).pluginType).toBe('core-plugin');
    }
  });
});
