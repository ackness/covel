...
import type { LLMAdapter, LLMRequest, LLMResponse } from '../src/llm-adapter.js';
...
function manifest(name: string, overrides: Partial<RuntimeManifest>): RuntimeManifest {
  return {
    name,
...
async function seedNpcGraph(store: DataStore, sessionId: string): Promise<void> {
...
describe('npc-graph core plugin write-read-inject path', () => {
  it('commits extractor graph writes, retrieves matching facts, and injects them into narrator prompt', async () => {
    ...
    const retrieverOutput = result.runtimeResults[0]!.output as Record<string, unknown>;
