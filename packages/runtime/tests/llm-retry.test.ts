...
import type { LLMAdapter, LLMMessage, LLMResponse, LLMStreamEvent } from '../src/llm-adapter.js';
...
describe('buildRetryPolicy', () => {
  ...
  it('clamps maxRetries to [0, 5]', () => {
    expect(buildRetryPolicy({ runtimeTimeoutMs: 60_000, maxRetries: -1 }).maxRetries).toBe(0);
    expect(buildRetryPolicy({ runtimeTimeoutMs: 60_000, maxRetries: 99 }).maxRetries).toBe(5);
  });
...
describe('isTransientError', () => {
  it('classifies common transient failures', () => {
    ...
    expect(isTransientError(new Error('PROVIDER_ERROR: upstream 502'))).toBe(true);
...
describe('detectToolLoop', () => {
  ...
  it('returns false when threshold is 0', () => {
    expect(detectToolLoop([mk('x', '{}'), mk('x', '{}'), mk('x', '{}')], 0)).toBe(false);
  });
  ...
  it('returns true for N identical trailing calls', () => {
    expect(detectToolLoop([mk('a', '{}'), mk('b', '{}'), mk('b', '{}'), mk('b', '{}')], 3)).toBe(true);
  });

  it('ignores earlier dissimilar calls', () => {
    expect(detectToolLoop([mk('a', '{}'), mk('a', '{}'), mk('b', '{}')], 3)).toBe(false);
  });

  it('treats different arg strings as different calls', () => {
    expect(detectToolLoop([mk('x', '{"v":1}'), mk('x', '{"v":2}'), mk('x', '{"v":1}')], 3)).toBe(false);
  });
});
...
describe('callLLMWithRetry', () => {
  it('returns the response on the first successful attempt', async () => {
    const llm = createScriptedLLM([{ kind: 'ok', response: okResponse('hello') }]);
  ...
  it('retries on transient error and succeeds', async () => {
    ...
    expect(retryMessages[retryMessages.length - 1]!.content).toContain('[retry');
  });
  ...
  it('surfaces the cause message in the wrapper error', async () => {
    ...
    const policy = buildRetryPolicy({ runtimeTimeoutMs: 10_000, maxRetries: 0 });
  ...
  it('does not retry an unknown / non-transient error', async () => {
    ...
    const policy = buildRetryPolicy({ runtimeTimeoutMs: 10_000, maxRetries: 3 });
