...
describe('TurnExecutor EventBus Bridge', () => {
  ...
  it('should emit turn.started and turn.completed events when eventBus is provided', async () => {
    ...
    expect((turnStarted!.payload as Record<string, unknown>).turnId).toBe('turn-1');

    expect(turnCompleted).toBeDefined();
    expect(turnCompleted!.topic).toBe('game');
    expect((turnCompleted!.payload as Record<string, unknown>).durationMs).toBeGreaterThanOrEqual(0);
  });
  ...
  it('should emit runtime.failed when runtime fails', async () => {
    ...
    failingLLM.generate = async () => { throw new Error('LLM exploded'); };
    ...
    expect((runtimeFailed!.payload as Record<string, unknown>).error).toContain('LLM exploded');
  });
