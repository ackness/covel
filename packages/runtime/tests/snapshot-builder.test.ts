  ...
  function createMockStore() {
    return {
      ...
      listMessages: vi.fn().mockResolvedValue([
        { id: 'm1', role: 'user', content: 'Hello', metadata: { turnId: 't1' }, createdAt: '2026-01-01' },
        { id: 'm2', role: 'assistant', content: 'Story text.', metadata: { turnId: 't1', runtimeId: 'narrator', kind: 'story' }, createdAt: '2026-01-01' },
      ]),
      listCharacters: vi.fn().mockResolvedValue([
        { id: 'c1', name: 'Hero', type: 'player', description: 'The protagonist', fields: { class: 'warrior' } },
      ]),
      listStateSchemas: vi.fn().mockResolvedValue([
        { tableName: 'world' },
        { tableName: 'player' },
      ]),
      listStateEntries: vi.fn().mockImplementation((_sid: string, table: string) => {
        if (table === 'world') return Promise.resolve([
          { tableName: 'world', fieldName: 'weather', value: 'rain' },
          { tableName: 'world', fieldName: 'time', value: 'night' },
        ]);
        if (table === 'player') return Promise.resolve([
          { tableName: 'player', fieldName: 'hp', value: 100 },
        ]);
        return Promise.resolve([]);
      }),
      listTraceEvents: vi.fn().mockResolvedValue([
        { type: 'runtime.started', turnId: 't1', payload: { runtimeId: 'narrator' }, createdAt: '2026-01-01' },
        { type: 'runtime.completed', turnId: 't1', payload: { runtimeId: 'narrator', durationMs: 500 }, createdAt: '2026-01-01' },
      ]),
    };
  }
