...
describe('executeTurn: output.schema.json', () => {
  ...
  it('enriches schema-prose failure with required fields, full LLM output, and diagnostic', async () => {
    ...
    const llm = new CapturingLLM();
    const fullProse = '日落渡口。一身旧道袍的少年弟子盘膝坐于石砌渡口的末级台阶上'.repeat(20);
    ...
    expect(failure?.error).toContain('expected a JSON envelope per output.schema');
    ...
    const diagnostic = output?.diagnostic as Record<string, unknown> | undefined;
    expect(diagnostic?.kind).toBe('schema-validation-prose');
    expect(diagnostic?.requiredFields).toEqual(['prompt', 'promptMode', 'events']);
    expect(diagnostic?.schemaTitle).toBe('openai-image-gen/prompt-generator output');
  ...
  it('handles malformed schemas (no required[]) gracefully on prose failure', async () => {
    ...
    const diagnostic = (failure?.output as Record<string, unknown>)?.diagnostic as Record<string, unknown>;
