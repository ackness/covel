...
describe('start-game flow scenario at runtime level', () => {
  it('start_session bootstrap keeps player history clean while the character form is pending', async () => {
    ...
    const result = await runTurn(store, deps, { turnId: 'turn-bootstrap', playerMessage: '' });

    expect(result.pendingInputs?.map((pending) => pending.interaction.interactionId)).toEqual([
      'form-char-creation',
    ]);
