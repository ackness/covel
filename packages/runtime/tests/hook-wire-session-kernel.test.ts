...
function makeProposal(type: Proposal['type'], payload: Record<string, unknown>, suffix: string): Proposal {
  return {
    id: `prop-${suffix}`,
    type,
...
describe('session-kernel hook wire-in', () => {
  describe('No-pipeline path', () => {
    it('commits normally without hookPipeline — no side effects', async () => {
      ...
      const proposals = [
        makeProposal('narrative.append', { content: 'hello', kind: 'story' }, 'a'),
  ...
  describe('PreStateCommit hook', () => {
    it('skips proposal when PreStateCommit aborts, commits the rest', async () => {
      ...
      const proposals = [
        ...
        makeProposal('state.patch', { table: 'stats', field: 'hp', value: 50 }, 'b'),
        makeProposal('narrative.append', { content: 'also ok', kind: 'story' }, 'c'),
      ];
  ...
  describe('PostStateCommit hook', () => {
    it('fires after successful commit with the proposal and result', async () => {
      ...
      hookPipeline.register({ id: 'test:PostStateCommit:0', event: 'PostStateCommit', handler });

      const pipeline = createCommitPipeline(store, hookPipeline);
      await pipeline.commit(makeProposal('narrative.append', { content: 'hello', kind: 'story' }, 'p'));

      expect(handler).toHaveBeenCalledOnce();
      const [_ctx, payload] = handler.mock.calls[0] as [unknown, { proposal: Proposal; result: { committed: boolean } }];
    ...
    it('abort from PostStateCommit does not undo the commit', async () => {
      ...
      hookPipeline.register({
        ...
        handler: vi.fn().mockResolvedValue({ action: 'abort', reason: 'audit failed' }),
      });

      const pipeline = createCommitPipeline(store, hookPipeline);
      const result = await pipeline.commit(makeProposal('narrative.append', { content: 'written', kind: 'story' }, 'q'));
