...
describe("normalizeOutput", () => {
  describe("narrative.append", () => {
    ...
    it('does NOT emit narrative.append for non-story runtimes (kind defaults to "plugin")', () => {
      ...
      expect(
        proposals.filter((p) => p.type === "narrative.append"),
      ).toHaveLength(0);
    });
    ...
    it("should skip narrativeTemplate (contains placeholders, not displayable)", () => {
      ...
      const narrativeProposals = proposals.filter(
        (p) => p.type === "narrative.append",
      );
  ...
  describe("pluginData", () => {
    ...
    it("produces no proposal when pluginData is absent, empty, or entirely invalid", () => {
      expect(
        normalizeOutput({}, SOURCE, TURN_ID, SESSION_ID).filter(
  ...
  describe("notifications", () => {
    ...
    it("produces notifications in addition to narrativeOutput (story only)", () => {
      ...
      expect(proposals).toHaveLength(2);
      const kinds = proposals.map((p) => ({
        type: p.type,
        kind: (p.payload as { kind?: string }).kind,
      }));
...
describe("createCommitPipeline", () => {
  ...
  describe("narrative.append commit", () => {
    it("should persist to messages table and emit narrative.completed event", async () => {
      ...
      const proposal = makeProposal("narrative.append", {
        content: "The hero advances.",
        kind: "story",
      });
  ...
  describe("phase.transition commit (removed post turn-band migration)", () => {
    it("should return committed: false because phase.transition is no longer a known proposal type", async () => {
      const store = createMockStore();
      const pipeline = createCommitPipeline(store as any);
      const proposal = makeProposal("phase.transition" as any, {
        phase: "playing",
      });
  ...
  describe("event.emit commit", () => {
    it("should persist event and emit event.emitted", async () => {
      const store = createMockStore();
      const pipeline = createCommitPipeline(store as any);
      const proposal = makeProposal("event.emit", {
  ...
  describe("character.upsert commit", () => {
    ...
    it("validates required fields", async () => {
      ...
      const result = await pipeline.commit(
        makeProposal("character.upsert", { id: "char-missing-name" }),
      );
  ...
  describe("CommitPipeline trace emissions (block/state)", () => {
    it("emits block.emitted when an interaction.request proposal commits", async () => {
      ...
      expect((blockEvent!.payload.block as Record<string, unknown>).type).toBe(
        "interactive_form",
      );
    });
    ...
    it("emits ui.rendered and ui.part.update when a ui.render proposal commits", async () => {
      ...
      expect(result.committed).toBe(true);
      expect(
        emitter.events.find((e) => e.type === "ui.rendered"),
      ).toBeDefined();
      expect(
        emitter.events.find((e) => e.type === "ui.part.update")?.payload.part,
      ).toMatchObject({
        id: "p1",
        status: "streaming",
      });
    ...
    it("emits state.patch.applied when a state.patch proposal commits", async () => {
      ...
      expect(result.committed).toBe(true);
      const stateEvent = emitter.events.find(
        (e) => e.type === "state.patch.applied",
      );
      ...
      expect(
        (stateEvent!.payload.patch as Record<string, unknown>).packageName,
      ).toBe("world");
      expect(
        (stateEvent!.payload.patch as Record<string, unknown>).summary,
      ).toBe("weather");
    });
    ...
    it("does not emit block.emitted or state.patch.applied for narrative proposals", async () => {
      ...
      expect(
        emitter.events.find((e) => e.type === "block.emitted"),
      ).toBeUndefined();
      expect(
        emitter.events.find((e) => e.type === "state.patch.applied"),
      ).toBeUndefined();
    });
    ...
    it("emits asset.generated when an asset.generate proposal commits", async () => {
      ...
      expect(result.committed).toBe(true);
      const assetEvent = emitter.events.find(
        (e) => e.type === "asset.generated",
      );
      ...
      expect(
        (assetEvent!.payload.asset as Record<string, unknown>).modality,
      ).toBe("image");
    });
  });
});
...
describe("processRuntimeResult", () => {
  ...
  it("commits a fake runtime asset output through the event and trace path", async () => {
    ...
    expect(store.addMessage).toHaveBeenCalledOnce();
    expect(store.addTraceEvent).toHaveBeenCalledWith(
    ...
    expect(
      emitter.events.find((e) => e.type === "asset.generated"),
    ).toBeDefined();
  });
  ...
  it("commits multiple asset.generate outputs in assetGenerations[] order", async () => {
    ...
    expect(
      events.map(
        (event) => (event.payload.asset as Record<string, unknown>).ref,
      ),
    ).toEqual([MEDIA_REF, MEDIA_REF_2]);
    expect(
      store.addMessage.mock.calls.map(
        (call) => call[0].metadata.block.data.ref,
      ),
    ).toEqual([MEDIA_REF, MEDIA_REF_2]);
  });
  ...
  it("should surface failed proposals when commit returns committed:false", async () => {
    ...
    const output = await processRuntimeResult(
      result,
      store as any,
      SESSION_ID,
      "story",
...
describe("createTraceRecorder", () => {
  ...
  it("should record runtime.completed trace events", async () => {
    ...
    await recorder.runtimeCompleted({
      runtimeId: "narrator",
      pluginId: "narrator",
      status: "success",
      durationMs: 1500,
