...
import { createTurnEmitter, createNoopTurnEmitter } from '../src/turn-emitter.js';
...
function makeBusSpy(): EventBus & { emitted: Array<{ type: string; payload: unknown }> } {
  const emitted: Array<{ type: string; payload: unknown }> = [];
  const bus: EventBus = {
    emit(ev) {
      emitted.push({ type: String((ev.payload as Record<string, unknown>)._subType ?? ''), payload: ev.payload });
    },
    onEmit: () => () => undefined,
...
describe('TurnEmitter', () => {
  it('persists to store and broadcasts on eventBus with monotonic seq', async () => {
    ...
    const emitter = createTurnEmitter({ store, eventBus: bus, sessionId: 'S', turnId: 'T' });
  ...
  it('tolerates store failure (warns but does not throw)', async () => {
    const store = { addTraceEvent: vi.fn(async () => { throw new Error('db down'); }) };
    const bus = makeBusSpy();
    const emitter = createTurnEmitter({ store, eventBus: bus, sessionId: 'S', turnId: 'T' });
