...
describe('scheduleByPriority — exact boundary 99 vs 100', () => {
  it('priority 99 is Pre-Game (turn 0 only)', () => {
    ...
    expect(scheduleByPriority([m], 0).flatMap((g) => g.runtimes.map((r) => r.name))).toEqual(['edge-99']);
    expect(scheduleByPriority([m], 1).flatMap((g) => g.runtimes.map((r) => r.name))).toEqual([]);
  });
  ...
  it('priority 100 is main-loop (turn ≥ 1 only)', () => {
    ...
    expect(scheduleByPriority([m], 1).flatMap((g) => g.runtimes.map((r) => r.name))).toEqual(['edge-100']);
  });

  it('priority 0 is the lowest valid Pre-Game value', () => {
    const m = rt('zero', 0);
    expect(scheduleByPriority([m], 0).flatMap((g) => g.runtimes.map((r) => r.name))).toEqual(['zero']);
  });
...
describe('scheduleByPriority — priority undefined safeguard', () => {
  ...
  it('does not let priority=undefined poison a mixed input', () => {
    ...
    expect(groups.flatMap((g) => g.runtimes.map((r) => r.name))).toEqual(['valid', 'audit']);
  });
});
...
describe('scheduleByPriority — input safety', () => {
  ...
  it('returns deterministic same-priority order for shuffled inputs', () => {
    ...
    expect(a[0].runtimes.map((r) => r.name)).toEqual(b[0].runtimes.map((r) => r.name));
    expect(a[0].runtimes.map((r) => r.name)).toEqual(['first', 'second', 'third']);
  });
