...
describe('HookPipeline', () => {
  ...
  describe('replace semantics (shallow merge)', () => {
    it('merges replace patch into payload for downstream handlers', async () => {
      const h1 = vi.fn().mockResolvedValue({ action: 'continue', replace: { x: 99 } });
      const h2 = vi.fn().mockResolvedValue({ action: 'continue' });

      pipeline.register({ id: 'test:PreToolUse:0', event: 'PreToolUse', handler: h1 });
      pipeline.register({ id: 'test:PreToolUse:1', event: 'PreToolUse', handler: h2 });
  ...
  describe('abort semantics', () => {
    it('stops the chain immediately on abort and returns abort result', async () => {
      const h1 = vi.fn().mockResolvedValue({ action: 'abort', reason: 'blocked' });
      const h2 = vi.fn().mockResolvedValue({ action: 'continue' });
      ...
      expect((result as { action: 'abort'; reason: string }).reason).toBe('blocked');
    ...
    it('emits hook.aborted event to eventBus on abort', async () => {
      ...
      bus.onEmit((e) => { emitted.push(e.type); });
  ...
  describe('timeout', () => {
    it('aborts with timeout reason when handler exceeds timeoutMs', async () => {
      const slowHandler = vi.fn().mockImplementation(
        () => new Promise<HookResult<object>>((resolve) => setTimeout(() => resolve({ action: 'continue' }), 500)),
      );
    ...
    it('emits hook.timeout event to eventBus on timeout', async () => {
      ...
      pipeline.register({
        id: 'test:TurnStart:0',
        event: 'TurnStart',
        handler: vi.fn().mockImplementation(() => new Promise(() => { /* never resolves */ })),
        timeoutMs: 10,
      });
  ...
  describe('thrown exception', () => {
    it('converts thrown exception to abort with error message', async () => {
      ...
      const result = await pipeline.run('PreStateCommit', makeCtx('PreStateCommit'), {});
      expect(result.action).toBe('abort');
      expect((result as { action: 'abort'; reason: string }).reason).toBe('something went wrong');
    });
    ...
    it('emits hook.error event to eventBus on thrown exception', async () => {
      ...
      await pipeline.run('PreStateCommit', makeCtx('PreStateCommit'), {}, { eventBus: bus });
  ...
  describe('match filter', () => {
    it('skips handler when match filter returns false', async () => {
      ...
      await pipeline.run('PreToolUse', makeCtx('PreToolUse'), { name: 'other-tool' });
      expect(handler).not.toHaveBeenCalled();
    });
    ...
    it('runs handler when match filter returns true', async () => {
      ...
      await pipeline.run('PreToolUse', makeCtx('PreToolUse'), { name: 'special-tool' });
      expect(handler).toHaveBeenCalledOnce();
    });
  });
  ...
  describe('all 8 HookEvent types have working emit sites', () => {
    const events: HookEvent[] = [
      ...
      'PreToolUse', 'PostToolUse', 'PreStateCommit', 'PostStateCommit', 'TurnStop',
    ];
