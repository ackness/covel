...
const EVENT_TOPIC = "image.generate.requested";
...
export default function ({ tool, z, shortId }) {
  ...
  return [
    tool({
      name: "submit-dashscope-text-prompt",
      description:
        "Submit one finished natural-language image prompt. This stores the prompt and starts DashScope image generation; call it exactly once for promptMode=text.",
      parameters: z.object({
    ...
    tool({
      name: "submit-dashscope-structured-prompt",
      description:
        "Submit one finished structured image prompt as a JSON object. This stores and serializes it, then starts DashScope image generation; call it exactly once for promptMode=image-json.",
      parameters: z.object({
        prompt: z
