...
export default function ({ tool, z, shortIdBatch, store }) {
  ...
  const edgeInputSchema = z.object({
    sourceName: z.string().min(1).describe('源节点的 name — 可指向新节点或已存在节点'),
  ...
  return tool({
    ...
    execute: async (params, context) => {
      ...
      const existingNodeRows =
        (await store.listPluginData(context.sessionId, context.pluginId, 'nodes')) ?? [];
      /** @type {Map<string, any>} */
      ...
      for (const incoming of incomingNodes) {
        ...
        if (existing) {
          ...
          nodeResults.push({ id: existing.id, name: existing.name, status: 'updated' });
        } else {
      ...
      const existingEdgeRows =
        (await store.listPluginData(context.sessionId, context.pluginId, 'edges')) ?? [];
      /** @type {Set<string>} */
      ...
      for (const [indexKey, bucket] of adjacencyUpdates) {
        ...
        const merged = Array.from(new Set([...prev, ...bucket.byNode.get(indexKey)]));
        pluginDataWrites.push({
          id: crypto.randomUUID(),
          sessionId: context.sessionId,
          pluginId: context.pluginId,
