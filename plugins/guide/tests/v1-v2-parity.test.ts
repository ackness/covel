/**
 * V1/V2 prompt-assembler parity test for `guide` (S2-T4).
 *
 * Verifies that after migrating guide to `promptVersion: 2`, the V2
 * assembler produces semantically equivalent prompts to V1 for the same
 * `TurnInput` + completed results from narrator.
 *
 * Unlike narrator, guide consumes an inject from an upstream
 * runtime (`narrator`). Both paths must surface that inject verbatim
...
function makeBaseParams(
	...
	return {
		...
		completedResults: new Map([["narrator", makeNarratorResult()]]),
		config: {},
		messageHistory: [
			{ role: "user", content: "I leave the inn." },
			{
				role: "assistant",
				content: "The mist wraps around your boots as you step outside.",
			},
		],
		sessionMeta: {
			turnNumber: 7,
...
describe("guide V1/V2 prompt parity", () => {
