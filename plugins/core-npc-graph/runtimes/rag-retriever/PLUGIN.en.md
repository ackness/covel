...
6. Emits `npcContext` (a markdown list) for `core-narrator` to consume via `input.inject`

When the graph is empty or no node was hit, the output is `npcContext: ""` and the `core-narrator` prompt naturally skips the corresponding section.
