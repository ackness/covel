...
describe('character-presence handler', () => {
  it('saves presence refs by character id', async () => {
    ...
    expect(proposals[0]).toMatchObject({
      type: 'plugin.data',
      source: { pluginId: 'character-presence', runtimeId: 'character-presence' },
      sessionId: 'sess-presence',
      turnId: 'turn-presence',
      payload: {
        namespace: 'presence',
  ...
  it('rejects malformed presence payloads', async () => {
    await expect(handler(ctx({ presenceJson: '{bad json' })))
      .rejects.toThrow('manualPayload.presenceJson must be valid JSON');

    await expect(handler(ctx({ presence: { avatar } })))
      .rejects.toThrow('presence.characterId must be a non-empty string');
    ...
    }))).rejects.toThrow('presence.avatar.id must be a 64-character lowercase sha256 hex string');
