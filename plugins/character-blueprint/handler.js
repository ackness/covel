...
export default async function handler(ctx) {
  ...
  const shouldInstantiate = payload.instantiate === true || blueprint.instantiate !== undefined;
...
function normalizeBlueprint(value) {
  ...
  if (!BLUEPRINT_ID_PATTERN.test(id)) {
    throw new Error('blueprint.id must be 1-128 characters using letters, digits, underscore, or hyphen');
  }
...
function characterIdForBlueprint(blueprint) {
  ...
  if (instantiate && typeof instantiate === 'object' && !Array.isArray(instantiate)) {
    const characterId = /** @type {Record<string, unknown>} */ (instantiate).characterId;
