...
describe('character-blueprint handler', () => {
  ...
  it('rejects malformed blueprintJson payloads', async () => {
    await expect(handler(ctx({ blueprintJson: '{bad json' })))
      .rejects.toThrow('manualPayload.blueprintJson must be valid JSON');
  });


  it('rejects malformed blueprint payloads', async () => {
    await expect(handler(ctx({ blueprint: { id: 'missing-name' } })))
