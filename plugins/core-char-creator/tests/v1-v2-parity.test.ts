/**
 * V1/V2 prompt-assembler parity test for `core-char-creator` (S3-T5a).
 *
 * core-char-creator is a multi-runtime plugin with two PLUGIN.md files:
...
function makeCharacterTrackerParams(
  ...
  return {
    promptTemplate,
    manifest,
    turnInput: makeTurnInput({ turnId: 'turn-3', playerMessage: 'I greet Elina.' }),
    completedResults: new Map([['core-narrator', makeNarratorResult('turn-3')]]),
    config: {
      worldSchema: {
        'character-attributes': {
          version: 1,
          attributes: [
            { id: 'hp', name: '生命值', type: 'number', defaultValue: 100, category: 'stats' },
          ],
        },
      },
    },
...
describe('core-char-creator/player-init V1/V2 prompt parity', () => {
...
describe('core-char-creator/character-tracker V1/V2 prompt parity', () => {
  it('manifest declares promptVersion: 2', () => {
  ...
  it('V2 gate requires both env flag AND promptVersion: 2', () => {
    const parsed = loadCharacterTracker();
    const v1Manifest = { ...parsed.manifest, promptVersion: 1 as const };
    const params = makeCharacterTrackerParams(parsed.promptTemplate, v1Manifest);
