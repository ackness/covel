...
describe('chat foundation manifests', () => {
  ...
  beforeAll(async () => {
    ...
    chatNarrator = (await loadPluginManifest(chatNarratorDiscovery))[0].manifest;
    loadedSceneCast = await loadRuntime(sceneCastDiscovery, sceneCast.name);
    loadedChatNarrator = await loadRuntime(chatNarratorDiscovery, chatNarrator.name);
  });
  ...
  it('loads chat-mode-narrator with active cast runtime injection', () => {
    ...
    expect(chatNarrator.capabilities).toEqual(expect.arrayContaining(['narrative', 'chat-mode']));
    ...
    expect(loadedChatNarrator.promptTemplate).toContain('{{ inputs.scene-cast.scene-cast.activeCastContext }}');
  });
  ...
  it('schedules scene-cast before chat-mode-narrator in the DAG runtime layer', () => {
    const { groups, error } = scheduleByDag([
      ...
      chatNarrator,
      {
        ...
        input: {
          inject: [
            { kind: 'runtime', from: 'chat-mode-narrator', field: 'narrativeOutput', as: '<narrator-output>' },
          ],
        },
      },
    ]);
    ...
    expect(groups.map((group) => group.runtimes.map((runtime) => runtime.name))).toEqual([
      ['scene-cast'],
      ['chat-mode-narrator'],
      ['guide'],
    ]);
  ...
  it('selects mentioned NPCs and writes active cast plugin data', async () => {
    const store = {
      async listCharacters(sessionId) {
        ...
        return [
          { id: 'player-1', name: 'Player', type: 'player', description: 'The player character' },
          { id: 'npc-1', name: 'Mira', type: 'npc', description: 'A wary smuggler with a soft voice', fields: { mood: 'guarded' } },
          { id: 'npc-2', name: 'Sol', type: 'npc', description: 'A temple archivist' },
        ];
      },
      async listTurnMessages(sessionId, opts) {
        ...
        return [
          { content: 'Mira studies the locked door while Sol waits near the altar.' },
        ];
      },
      async getPluginData() {
        return null;
    ...
    const store = {
      async listCharacters() {
        return [
          ...
          { id: 'npc-1', name: 'Ari', type: 'npc', description: 'A quiet medic', fields: { mood: 'calm' } },
          { id: 'npc-2', name: 'Bex', type: 'npc', description: 'A bright scout' },
        ];
      },
