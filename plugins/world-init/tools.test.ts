...
describe('world-init local tools', () => {
    ...
    expect(proposals[0]).toMatchObject({
      ...
      payload: {
        ...
        value: {
          version: 1,
          attributes: [
            expect.objectContaining({ id: 'hp', type: 'number', category: 'stats' }),
            expect.objectContaining({ id: 'relationships', type: 'map', category: 'social' }),
          ],
        },
      },
    });
  ...
  it('queues plugin-data and lorebook proposals for world entries', async () => {
    ...
    expect(proposals[0]).toMatchObject({
      ...
      payload: {
        items: [
          { namespace: 'entries', key: 'geography', value: { regions: ['云梦泽'] } },
          { namespace: 'entries', key: 'factions', value: { groups: ['青萍宗'] } },
        ],
      },
    });
  ...
  it('commits queued world entries atomically through the kernel path', async () => {
    ...
    const commitResults = await createCommitPipeline(store).commitAll(getPendingProposals(result));
    expect(commitResults.every((item) => item.committed)).toBe(true);

    const pluginData = await store.listPluginData(context.sessionId, context.pluginId, 'entries');
    expect(pluginData.map((row) => row.key).sort()).toEqual(['factions', 'geography']);
    ...
    expect(lorebook[0].content).toBe('[geography]\n{\n  "regions": [\n    "云梦泽"\n  ]\n}');
