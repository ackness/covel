...
export default async function handler(ctx) {
  ...
  throw new Error('manualPayload.action must be createCandidates or acceptCandidate');
}

function createCandidates(ctx, payload) {
  const now = new Date().toISOString();
  const targetTurnId = normalizeTurnId(payload.turnId ?? ctx.turnId);
  const baseText = normalizeOptionalString(payload.baseText, 'baseText') ?? normalizeFallbackText(ctx.playerMessage);
  ...
  const selectedCandidateId = normalizeOptionalString(payload.selectedCandidateId, 'selectedCandidateId')
    ?? candidates[0]?.id;
  const turnRecord = {
    schemaVersion: 1,
    turnId: targetTurnId,
  ...
  return withPendingProposals(
    ...
    [
      makePluginDataBatchProposal(ctx, now, [
        ...
        { namespace: MESSAGE_NAMESPACE, key: targetTurnId, value: messageState },
      ]),
    ],
  );
}
...
async function acceptCandidate(ctx, payload) {
  const now = new Date().toISOString();
  const targetTurnId = normalizeTurnId(payload.turnId ?? ctx.turnId);
  const candidateId = normalizeRequiredString(payload.candidateId, 'candidateId');
  const existingTurnRecord = await readTurnRecord(ctx.store, ctx.sessionId, ctx.pluginId, targetTurnId);
  if (!existingTurnRecord) {
    throw new Error('branch-reply turn record was not found');
  }
  const candidateSet = existingTurnRecord;
  const accepted = candidateSet.candidates.find((candidate) => candidate.id === candidateId);
  if (!accepted) {
    throw new Error('manualPayload.candidateId must reference an existing candidate');
  }
  const acceptedText = normalizeOptionalString(payload.text, 'text') ?? accepted.text;
  ...
  return withPendingProposals(
    ...
    [
      makePluginDataBatchProposal(ctx, now, [
        { namespace: TURNS_NAMESPACE, key: targetTurnId, value: nextTurnRecord },
        { namespace: MESSAGE_NAMESPACE, key: targetTurnId, value: messageState },
      ]),
    ],
  );
...
function normalizeTurnId(value) {
  ...
  if (!TURN_ID_PATTERN.test(turnId)) {
    throw new Error('turnId must be 1-128 characters using letters, digits, underscore, dot, colon, or hyphen');
  }
  return turnId;
}
...
function normalizeCount(value) {
  ...
  if (!Number.isInteger(value) || value < 1 || value > MAX_COUNT) {
    throw new Error(`manualPayload.count must be an integer from 1 to ${MAX_COUNT}`);
  }
  return value;
}
...
function selectCandidateId(candidates, requestedId) {
  ...
  throw new Error('manualPayload.selectedCandidateId must reference an existing candidate');
}

function buildCandidates({ turnId, baseText, count, variants, now }) {
  const sourceTexts = variants?.length ? variants.slice(0, MAX_COUNT) : deterministicTexts(baseText, count);
...
function deterministicTexts(baseText, count) {
  ...
  return Array.from({ length: count }, (_, index) => templates[index](baseText));
}
...
async function readTurnRecord(store, sessionId, pluginId, turnId) {
  ...
  const row = s.getPluginData.length <= 2
  ...
  const candidates = record.candidates.flatMap((item) => {
    ...
    if (!id || !text || id.length > MAX_TEXT_LENGTH || text.length > MAX_TEXT_LENGTH) return [];
    return [{
      ...candidate,
      id,
      text,
