...
describe('generate-scene-prompts', () => {
  it('writes fixed-slot message data for the plugin-message UI', async () => {
    ...
    expect(items).toContainEqual({ namespace: 'message', key: '__turnId', value: 'turn-7' });
    expect(items).toContainEqual({ namespace: 'message', key: 'scene', value: '破庙暗影' });
    expect(items).toContainEqual({
      namespace: 'message',
      key: 'prompt2Text',
      value: '我低声问苏婉有没有认出这道符纹',
    });
    expect(items).toContainEqual({ namespace: 'message', key: 'prompt3Icon', value: 'zap' });
    expect(items).toContainEqual({ namespace: 'message', key: 'prompt6Text', value: '' });
  });
});
