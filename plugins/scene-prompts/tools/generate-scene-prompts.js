...
export default function ({ tool, z }) {
  const promptSchema = z.object({
    kind: z.enum(['observe', 'ask', 'act', 'social'])
  ...
  return tool({
    ...
    description: '生成场景化快捷回复。写入 message 命名空间，前端以 ui.message 渲染为可草拟或直接发送的提示按钮。',
    parameters: z.object({
      scene: z.string().min(1).max(40).describe('当前场景或决策点标题'),
      prompts: z.array(promptSchema).min(3).max(6).describe('3-6 条可直接发送的玩家行动短句'),
    }),
    execute: async (params, context) => {
      ...
      for (let i = 0; i < 6; i += 1) {
        ...
        items.push(
          { namespace: 'message', key: `prompt${slot}Text`, value: prompt?.text ?? '' },
          { namespace: 'message', key: `prompt${slot}Kind`, value: prompt?.kind ?? '' },
          { namespace: 'message', key: `prompt${slot}Label`, value: prompt?.label ?? '' },
          { namespace: 'message', key: `prompt${slot}Icon`, value: prompt?.icon ?? '' },
          { namespace: 'message', key: `prompt${slot}Color`, value: prompt?.color ?? '' },
        );
      }
