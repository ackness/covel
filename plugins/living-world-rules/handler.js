...
const VALID_CATEGORIES = new Set(['character', 'scene', 'relationship', 'world', 'style']);
...
function normalizeRule(value) {
  ...
  if (!RULE_ID_PATTERN.test(id)) {
    throw new Error('rule.id must be 1-128 characters using letters, digits, underscore, or hyphen');
  }
  ...
  const category = normalizeOptionalEnum(input.category, VALID_CATEGORIES, 'rule.category');
  const budgetClass = normalizeOptionalEnum(input.budgetClass, VALID_BUDGET_CLASSES, 'rule.budgetClass');
  const keys = normalizeStringArray(input.keys);
  const insertionOrder = normalizeOptionalNumber(input.insertionOrder, 'rule.insertionOrder');
...
function normalizeCoordinate(value) {
  ...
  const input = /** @type {Record<string, unknown>} */ (value);
  const position = typeof input.position === 'string' ? input.position : 'after_plugin';
  if (!VALID_POSITIONS.has(position)) {
    throw new Error('rule.coordinate.position must be before_plugin, after_plugin, or at_depth');
  }
...
function ruleToLorebookEntry(rule, lorebookEntryId) {
  ...
  return {
    ...
    insertionOrder: typeof rule.insertionOrder === 'number' ? rule.insertionOrder : 500,
