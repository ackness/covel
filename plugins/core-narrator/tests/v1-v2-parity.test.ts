/**
 * V1/V2 prompt-assembler parity test for `core-narrator` (S2-T4).
 *
 * Verifies that after migrating core-narrator to `promptVersion: 2`, the V2
...
function makeNpcGraphResult(): RuntimeResult {
  return {
    ...
    runtimeId: 'core-npc-graph/rag-retriever',
    runId: 'run-1',
    turnId: 'turn-7',
    status: 'success',
    output: {
...
function makeBaseParams(
  ...
  return {
    ...
    completedResults: new Map([
      ['core-npc-graph/rag-retriever', makeNpcGraphResult()],
    ]),
    config: {
      worldLore:
...
describe('core-narrator V1/V2 prompt parity', () => {
