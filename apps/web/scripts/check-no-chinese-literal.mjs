...
function isWhitelisted(relPath) {
  return WHITELIST_PREFIXES.some((prefix) => relPath === prefix || relPath.startsWith(prefix));
}
...
function main() {
  ...
  for (const file of files) {
    ...
    for (const f of findings) {
      ...
      console.error(`${rel}:${f.line}:${f.col}: raw CJK literal — move to i18n dictionary or mark // i18n-allow`);
      // eslint-disable-next-line no-console
