...
const makeSuspension = (overrides: Partial<SuspensionRecord> = {}): SuspensionRecord => ({
  id: "susp-1",
  sessionId: "sess-1",
  turnId: "turn-1",
  runtimeId: "image-gen",
...
describe("session-store — F4 suspensions slice", () => {
  ...
  it("SET_SUSPENSIONS: hydrates the list from the server snapshot", () => {
    ...
    const next = reduce(empty, { type: "SET_SUSPENSIONS", suspensions: snapshot });
  ...
  it("REMOVE_SUSPENSION: filters out the matching id", () => {
    ...
    const next = reduce(initial, { type: "REMOVE_SUSPENSION", suspensionId: "drop" });
  ...
  it("REMOVE_SUSPENSION: is a no-op when the id isn't in the list", () => {
    ...
    const next = reduce(initial, { type: "REMOVE_SUSPENSION", suspensionId: "ghost" });
  ...
  it("runtime.completed: reads payload.status — 'suspended' no longer reported as 'completed'", () => {
    ...
    expect(resolveRuntimeCompletedStatus({ status: "suspended" })).toBe("suspended");
    expect(resolveRuntimeCompletedStatus({ status: "completed" })).toBe("completed");
    expect(resolveRuntimeCompletedStatus({ status: "failed" })).toBe("failed");
    expect(resolveRuntimeCompletedStatus({ status: "skipped" })).toBe("skipped");
  ...
  it("runtime.completed + UPSERT: writes 'suspended' into the execution step row", () => {
    ...
    const status = resolveRuntimeCompletedStatus({ status: "suspended", durationMs: 42 });
