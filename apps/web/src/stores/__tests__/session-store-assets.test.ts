...
function makeAsset(overrides: Partial<AssetGenerateView> = {}): AssetGenerateView {
  return {
    id: "prop-1",
    type: "asset.generate",
    sessionId: "sess-1",
...
describe("session-store — P0-b assetsByTurn slice", () => {
  it("ASSET_GENERATED appends to the matching turn bucket", () => {
    ...
    const next = reduce(empty, { type: "ASSET_GENERATED", turnId: "turn-1", asset: a });
  ...
  it("ASSET_GENERATED is idempotent on duplicate ids (eventBus double-delivery safety)", () => {
    ...
    const once = reduce(empty, { type: "ASSET_GENERATED", turnId: "turn-1", asset: a });
    const twice = reduce(once, { type: "ASSET_GENERATED", turnId: "turn-1", asset: a });
  ...
  it("ASSET_GENERATED never mutates the previous bucket (immutability check)", () => {
    ...
    const twice = reduce(once, { type: "ASSET_GENERATED", turnId: "turn-1", asset: b });
