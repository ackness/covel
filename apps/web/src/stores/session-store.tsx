...
function toExecutionStepStatus(
...
function applyPluginMessageSurface(
  state: SessionState,
  pluginId: string,
): SessionState {
...
// ── _jobs status-transition watcher ───────────────────────────────
//
// Detects terminal state changes in the framework-managed `_jobs` namespace
// (pending → done | failed) and emits a toast so the player gets feedback for
// long-running background work without having to monitor the right panel.
...
function collectJobTransitions(
  pluginId: string,
...
function emitJobTransitionToast(tr: JobTransition): void {
        ...
        if (res.ok) {
          ...
          if (keys && typeof keys === "object" && !Array.isArray(keys)) {
            ...
            if (
              ...
              Object.keys(validKeys).length > 0
            ) {
  ...
  const handleSseEvent = useCallback((envelope: api.SseEnvelope) => {
    ...
    switch (type) {
      ...
      case "narrative.delta": {
        ...
        const deltaKind =
          (payload.kind as string) ?? runtimeKindRef.current.get(runtimeId);
      ...
      case "asset.progress": {
        ...
        const progress: AssetProgressEvent = {
          ...
          ...(typeof payload.message === "string" && payload.message.length > 0
            ...
            : {}),
          ...(typeof payload.modality === "string" &&
          payload.modality.length > 0
    ...
    async (session: api.SessionRecord) => {
      ...
      try {
        ...
        if (snapshot.executionSteps.length > 0) {
          ...
          for (const evt of snapshot.executionSteps) {
            ...
            const status: ExecutionStep["status"] =
              evt.type === "runtime.completed"
                ? "completed"
                : evt.type === "runtime.failed"
                  ? "failed"
                  : evt.type === "runtime.skipped"
                    ? "skipped"
                    : "running";
            byKey.set(key, {
              runtimeId,
              ...
              durationMs:
                (p.durationMs as number | undefined) ?? prev?.durationMs,
              startedAt:
                evt.type === "runtime.started"
                  ? evt.timestamp
                  : prev?.startedAt,
            });
          }
      ...
      if (!snapshotLoaded) {
        ...
        if (messagesResult.status === "fulfilled") {
          const streamMessages: StreamMessage[] = messagesResult.value.map(
            (m) => ({
              id: m.id,
              role: m.role as "system" | "user" | "assistant",
              content: m.content,
      ...
      try {
        const { ids: blockIds, values: blockValues } =
          await ds.loadSubmittedBlocks(session.id);
        if (sessionIdRef.current !== targetSessionId) return;
      ...
      try {
        ...
        if (sessionIdRef.current !== targetSessionId) return;
        const migrated = (raw as Array<Record<string, unknown>>).map(
          (r): ExecutionStep => {
              ...
              legacyType === "runtime.completed"
                ...
                : legacyType === "runtime.failed"
                  ...
                  : legacyType === "runtime.skipped"
                    ? "skipped"
                    : ((r.status as ExecutionStep["status"] | undefined) ??
                      "completed");
            return {
              ...
              startedAt:
                (r.startedAt as string | undefined) ??
    ...
    const handleSubscriptionEvent = (event: SubscriptionEvent) => {
      switch (event.type) {
        ...
        case "plugin-data.changed": {
          ...
          if (pluginId && changes) {
            // Capture _jobs status prior to applying so we can detect terminal
            // transitions (pending → done|failed) and surface a toast. Without
            // this the player kicks off a background runtime (image gen, etc.),
            // sees the submission toast from PluginPanel, then nothing — they
            // have no idea whether the work succeeded until they navigate to
            // the right-panel gallery and notice an entry. This is generic to
            // every plugin that uses the framework `_jobs` namespace.
            const transitions = collectJobTransitions(pluginId, changes);
            dispatch({ type: "PLUGIN_DATA_CHANGED", pluginId, changes });
            applyPluginDataStoreChanges(pluginId, changes);
            for (const tr of transitions) emitJobTransitionToast(tr);
