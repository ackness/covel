...
function collectPluginMessageStates(
  ...
  const directTurnId = typeof namespaceState.__turnId === "string" ? namespaceState.__turnId : "";
...
function applyPluginMessageSurface(state: SessionState, pluginId: string): SessionState {
  ...
  const desiredIds = new Set(messageStates.map(({ turnId }) => `plugin-message:${pluginId}:${turnId}`));
  ...
  if (tr.nextStatus === "done") {
    ...
    emitToast(
      ...
      i18n.t("pluginJob.failed", {
        target,
        error: trimmedError || i18n.t("pluginJob.unknownError", { defaultValue: "unknown error" }),
