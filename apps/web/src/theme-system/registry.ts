...
function sortThemes(themes: ThemeDefinition[]): ThemeDefinition[] {
  return [...themes].sort((a, b) => {
    ...
    if (a.source === "builtin" && b.source === "builtin") {
      return BUILTIN_THEME_ORDER.indexOf(a.id) - BUILTIN_THEME_ORDER.indexOf(b.id);
    }
...
function buildRegistry(themes: ThemeDefinition[]): Map<string, ThemeDefinition> {
...
function registerAppearanceEntry(store: SettingsStoreApi): void {
  ...
  store.register({
    ...
    description: {
      ...
      "en-US": "Choose the active interface style. Imported custom themes appear here automatically.",
    },
    options,
  });
}
...
function toStoredTheme(theme: ThemeDefinition, fileName?: string): StoredCustomTheme {
  return {
    id: theme.id,
    label: theme.label,
...
export function syncThemeRegistry(store: SettingsStoreApi): ThemeDefinition[] {
  const customThemes = loadStoredCustomThemes(store).map<ThemeDefinition>((theme) => ({
    ...theme,
    source: "custom",
  }));
