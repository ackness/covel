...
import type { ThemeDefinition, ThemeScheme } from "./types.js";
 ...
 */
const UNSAFE_VALUE = /[;{}]/;
const GENERATED_HEADER = "/* Generated by Covel — Settings → Appearance */";
const SNAPSHOT_START = "/* covel-token-snapshot:start */";
const SNAPSHOT_END = "/* covel-token-snapshot:end */";

function sourceEffects(
  source: Pick<ThemeDefinition, "id" | "cssText">,
): string {
  let cssText = source.cssText.trim();
  if (cssText.startsWith(GENERATED_HEADER))
    cssText = cssText.slice(GENERATED_HEADER.length).trim();
  // Replace only our trailing snapshot, keeping the original effects once.
  const start = cssText.lastIndexOf(SNAPSHOT_START);
  if (start !== -1 && cssText.endsWith(SNAPSHOT_END))
    cssText = cssText.slice(0, start).trimEnd();
  return cssText;
}
...
export function buildThemeCss(
  ...
  sourceTheme?: Pick<ThemeDefinition, "id" | "cssText">,
): ThemeCssSnapshot {
  const root = document.documentElement;
  ...
  const sourceCss = sourceTheme ? sourceEffects(sourceTheme) : "";
  const effects =
    sourceCss && sourceTheme
      ? deriveThemeCss(sourceCss, sourceTheme.id, themeId)
      : "";
  const cssText = [
    GENERATED_HEADER,
    effects,
    ...
    .filter(Boolean)
    .join("\n\n");
  parseImportedThemeFile(cssText, `${themeId}.css`);
  return { cssText, schemes };
}
