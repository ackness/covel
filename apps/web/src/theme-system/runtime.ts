...
function mountThemeStyle(theme: ThemeDefinition): void {
  ...
  if (existing instanceof HTMLStyleElement) {
    if (existing.textContent !== theme.cssText) existing.textContent = theme.cssText;
    return;
  }
...
export function syncThemeStyles(themes: readonly ThemeDefinition[]): void {
  ...
  for (const style of document.querySelectorAll<HTMLStyleElement>("style[data-theme-style]")) {
