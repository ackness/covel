...
const Textarea: ComponentRenderer = ({ element, bindings }) => {
  const resolve = useI18nResolver();
  const placeholder = resolve(element.props?.placeholder);
  const label = resolve(element.props?.label);
  const value = element.props?.value as string ?? "";
  ...
  return (
    <div className="space-y-1">
      ...
      <textarea
        ...
        placeholder={placeholder}
        onChange={(e) => bindPath && set(bindPath, e.target.value)}
        className={clsx(inputBase, "min-h-[160px] resize-y font-mono leading-relaxed")}
      />
    </div>
  );
};
...
function normalizeBranchReplyCandidates(
  value: unknown,
): { turnId?: string; acceptedCandidateId?: string; candidates: BranchReplyCandidate[] } {
  ...
  const acceptedCandidateId =
    typeof data.acceptedCandidateId === "string" ? data.acceptedCandidateId : undefined;
 ...
 */
const BranchReplyCandidates: ComponentRenderer = ({ element }) => {
  const resolve = useI18nResolver();
  ...
  const payload =
    ...
    props.data ??
    props;
  const { turnId, acceptedCandidateId, candidates } = normalizeBranchReplyCandidates(payload);
  const pluginId = typeof props.pluginId === "string" ? props.pluginId : "branch-reply";
    ...
    try {
      ...
      if (res.status === "error") {
        ...
        if (runtimeError || res.abortReason) {
          emitToast("error", runtimeError ?? res.abortReason ?? "Branch reply action failed");
        }
      }
    } catch (err) {
  ...
  return (
    <div className="ui-band space-y-2.5" data-tone="muted">
      <div className="flex items-center justify-between gap-3">
        <span className="ui-eyebrow text-[11px] text-muted-foreground">{title}</span>
        {turnId && (
          <button
            type="button"
            onClick={() => void invokeBranchReplyAction("createCandidates", {
          ...
          >
            <Icons.RefreshCw className={clsx("w-3 h-3", pendingAction === "createCandidates" && "animate-spin")} />
          ...
          return (
            ...
            >
              <div className="flex items-start gap-2">
                ...
                {accepted && <Icons.Check className="w-3.5 h-3.5 shrink-0 text-primary mt-0.5" />}
              </div>
              <div className="flex flex-wrap items-center gap-1.5 pl-5">
                ...
                <button
                  type="button"
                  onClick={() => void invokeBranchReplyAction("acceptCandidate", {
                ...
                >
                  {pendingAction === "acceptCandidate" && <Icons.Loader2 aria-hidden="true" className="w-3 h-3 animate-spin" />}
                  {acceptLabel}
                </button>
                {(candidate.source || candidate.runtimeId) && (
                  <span className="font-mono text-[9px] text-muted-foreground/60">
                    {[candidate.source, candidate.runtimeId].filter(Boolean).join(" · ")}
                  </span>
                )}
              </div>
            </div>
