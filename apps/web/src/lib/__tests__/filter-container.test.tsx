...
const { resolvePath, valueToHaystack, filterItems } = __filterContainerInternals;
...
describe("FilterContainer pure helpers", () => {
  ...
  describe("filterItems", () => {
    ...
    it("returns all items when no filter or query", () => {
      const result = filterItems(FIXTURE_ITEMS, "", searchFields, filterField, "all");
      expect(result).toHaveLength(4);
    });

    it("filters by tab value", () => {
      const result = filterItems(FIXTURE_ITEMS, "", searchFields, filterField, "monster");
      expect(result.map((i) => (i as typeof FIXTURE_ITEMS[number]).key)).toEqual(["e1", "e4"]);
    });

    it("filters by search query against searchFields", () => {
      const result = filterItems(FIXTURE_ITEMS, "crystal", searchFields, filterField, "all");
      expect(result).toHaveLength(1);
      expect((result[0] as typeof FIXTURE_ITEMS[number]).key).toBe("e2");
    });

    it("matches search against array fields (tags)", () => {
      const result = filterItems(FIXTURE_ITEMS, "spectral", searchFields, filterField, "all");
      expect(result).toHaveLength(1);
      expect((result[0] as typeof FIXTURE_ITEMS[number]).key).toBe("e4");
    });

    it("combines tab + search filters (AND)", () => {
      const result = filterItems(FIXTURE_ITEMS, "goblin", searchFields, filterField, "monster");
      expect(result).toHaveLength(1);
      expect((result[0] as typeof FIXTURE_ITEMS[number]).key).toBe("e1");
    });

    it("returns empty when nothing matches the combined filter", () => {
      const result = filterItems(FIXTURE_ITEMS, "lava", searchFields, filterField, "all");
      expect(result).toHaveLength(0);
    });

    it("treats an undefined filterField as no tab filter", () => {
      const result = filterItems(FIXTURE_ITEMS, "", searchFields, undefined, "anything");
      expect(result).toHaveLength(4);
    });

    it("is case-insensitive for search", () => {
      const result = filterItems(FIXTURE_ITEMS, "GOBLIN", searchFields, filterField, "all");
...
const FILTER_TABS = [
  ...
  { value: "monster", label: { zh: "怪物", en: "Monsters" }, icon: "skull", color: "red" },
  { value: "item", label: { zh: "物品", en: "Items" }, icon: "gem", color: "blue" },
  { value: "location", label: { zh: "地点", en: "Locations" }, icon: "map-pin" },
];
...
describe("FilterContainer component", () => {
  ...
  it("shows the empty message when no items match", () => {
    renderFilterContainer({ emptyMessage: { zh: "什么都没找到", en: "Nothing found" } });
    const searchInput = screen.getByPlaceholderText("搜索…") as HTMLInputElement;
