...
describe("BranchReplyCandidates", () => {
  const candidateSet = {
    turnId: "turn-42",
    candidates: [
      { id: "turn-42-candidate-1", text: "Ask about the sealed door.", source: "deterministic" },
      { id: "turn-42-candidate-2", text: "Wait and watch the guard.", source: "deterministic" },
    ],
  };
  ...
  it("disables runtime actions when the message state has no turn id", () => {
    renderBranchReply({ candidates: [{ id: "candidate-1", text: "Draft only." }] });
  ...
  it("keeps draft and send on the local session path", () => {
    ...
    expect(sessionMock.current.upsertInteractionDraft).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        values: { text: "Ask about the sealed door.", candidateId: "turn-42-candidate-1" },
      }),
    );

    fireEvent.click(screen.getAllByRole("button", { name: "Send" })[1]);
    expect(sessionMock.current.sendMessage).toHaveBeenCalledWith("Wait and watch the guard.");
  });
});
