...
export function SuspensionsPanel({ suspensions, onResume, onCancel }: SuspensionsPanelProps) {
  ...
  return (
    <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">
      {suspensions.map((s) => (
        <SuspensionCard key={s.id} suspension={s} onResume={onResume} onCancel={onCancel} />
      ))}
    </div>
  );
}
...
function SuspensionCard({ suspension, onResume, onCancel }: SuspensionCardProps) {
  const { t } = useTranslation();
  ...
  const runtimeLabel = suspension.runtimeId === suspension.pluginId
    ? suspension.pluginId
    ...
    <div className="border border-border rounded-sm p-3 space-y-2 bg-card/40">
      <div className="flex items-start justify-between gap-3 text-xs">
        <div className="min-w-0 flex-1">
          ...
          {suspension.reason && (
            <p className="mt-1 text-muted-foreground break-words">{suspension.reason}</p>
          )}
        </div>
      ...
      <div className="flex gap-2 justify-end">
        <Button
          ...
          disabled={busy !== null}
        >
          {busy === "cancel" ? <Loader2 className="w-3 h-3 animate-spin" /> : t("session.suspensionCancelLabel")}
        </Button>
        <Button
          size="sm"
...
function formatRelativeTime(
  ...
  if (seconds < 60) return t("session.suspensionAgoSeconds", { count: seconds });
  const minutes = Math.floor(seconds / 60);
  if (minutes < 60) return t("session.suspensionAgoMinutes", { count: minutes });
