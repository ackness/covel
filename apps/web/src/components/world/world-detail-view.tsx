...
function GeographySection({ geo, t }: { geo: WorldGeography; t: (k: string) => string }) {
  return (
    <WorldDimensionSection title={t("world.geography")} icon={MapPin}>
      {geo.overview && <p className="mb-3 text-sm text-muted-foreground">{text(geo.overview)}</p>}
              ...
              {region.landmarks && region.landmarks.length > 0 && (
                <ul className="mt-2 list-disc pl-4">
                  {region.landmarks.map((lm, j) => (
                    <li key={j}>{text(lm.name)}{lm.description ? ` — ${text(lm.description)}` : ""}</li>
                  ))}
                </ul>
              )}
            </CardContent>
          ...
          <Card key={faction.id}>
            <CardHeader className="p-3 pb-1">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="secondary" className="text-xs">{faction.type}</Badge>
                <Badge variant="outline" className="text-xs">{faction.influence}</Badge>
              </div>
            </CardHeader>
              ...
              {faction.relations && faction.relations.length > 0 && (
                <ul className="mt-1 list-disc pl-4">
                  {faction.relations.map((rel, j) => (
                    <li key={j}>
                      <Badge variant="outline" className="mr-1 text-xs">{rel.type}</Badge>
                      {rel.targetId}
...
function PowerSystemSection({ ps, t }: { ps: WorldPowerSystem; t: (k: string) => string }) {
  return (
      ...
      <div className="space-y-2 text-sm">
        ...
        {ps.rules.length > 0 && (
          <ul className="list-disc pl-4 text-xs text-muted-foreground">
            {ps.rules.map((rule, i) => <li key={i}>{text(rule)}</li>)}
          </ul>
        )}
        {ps.tiers && ps.tiers.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {[...ps.tiers].sort((a, b) => a.rank - b.rank).map((tier, i) => (
...
function EconomySection({ economy, t }: { economy: WorldEconomy; t: (k: string) => string }) {
  return (
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {economy.resources.map((r, i) => (
                <Badge key={i} variant="outline" className="text-xs">{text(r)}</Badge>
              ))}
            </div>
          </div>
        )}
        {economy.tradeNotes && (
          <p className="text-xs text-muted-foreground">{text(economy.tradeNotes)}</p>
        )}
      </div>
    </WorldDimensionSection>
  );
...
function SocialSection({ social, t }: { social: WorldSocialStructure; t: (k: string) => string }) {
  return (
    <WorldDimensionSection title={t("world.socialStructure")} icon={Building2}>
      <div className="space-y-3 text-sm">
          ...
          <div>
            ...
            {social.races.map((race, i) => (
              <div key={i} className="mb-1 text-xs">
                ...
                {race.description && <span className="text-muted-foreground"> — {text(race.description)}</span>}
                {race.traits && race.traits.length > 0 && (
                  <div className="mt-0.5 flex flex-wrap gap-1">
                    {race.traits.map((tr, j) => (
                      <Badge key={j} variant="outline" className="text-xs">{text(tr)}</Badge>
                    ))}
                  </div>
                )}
              </div>
        ...
        {social.notes && <p className="text-xs text-muted-foreground">{text(social.notes)}</p>}
      </div>
    </WorldDimensionSection>
  );
}

function ToneSection({ tone, t }: { tone: WorldTone; t: (k: string) => string }) {
  return (
    <WorldDimensionSection title={t("world.tone")} icon={Palette}>
      <div className="space-y-2 text-sm">
        <div className="flex flex-wrap gap-1">
          ...
          <Badge variant="default" className="text-xs">{tone.contentRating}</Badge>
        </div>
        {tone.narrativeStyle && (
          <p className="text-xs text-muted-foreground">{text(tone.narrativeStyle)}</p>
        )}
...
function MechanicsSection({ mechanics, t }: { mechanics: WorldMechanics; t: (k: string) => string }) {
  return (
    <WorldDimensionSection title={t("world.mechanics")} icon={Gamepad2}>
      <div className="space-y-2 text-sm">
        <div className="flex flex-wrap gap-1">
          {mechanics.combatStyle && (
            <Badge variant="secondary" className="text-xs">{mechanics.combatStyle}</Badge>
          )}
          {mechanics.difficulty && (
            <Badge variant="outline" className="text-xs">{mechanics.difficulty}</Badge>
          )}
        </div>
        {mechanics.skillSystem && (
          <p className="text-xs text-muted-foreground">{text(mechanics.skillSystem)}</p>
        )}
        {mechanics.customRules && mechanics.customRules.length > 0 && (
          <ul className="list-disc pl-4 text-xs text-muted-foreground">
            {mechanics.customRules.map((rule, i) => <li key={i}>{text(rule)}</li>)}
          </ul>
        )}
      </div>
    </WorldDimensionSection>
...
function StartingSection({ sc, t }: { sc: WorldStartingConditions; t: (k: string) => string }) {
  return (
    <WorldDimensionSection title={t("world.startingConditions")} icon={Flag}>
      <div className="space-y-2 text-sm">
        ...
        {sc.playerConstraints && sc.playerConstraints.length > 0 && (
          <ul className="list-disc pl-4 text-xs text-muted-foreground">
            {sc.playerConstraints.map((c, i) => <li key={i}>{text(c)}</li>)}
          </ul>
        )}
        {sc.startingResources && Object.keys(sc.startingResources).length > 0 && (
          <div className="flex flex-wrap gap-1">
            {Object.entries(sc.startingResources).map(([name, amount]) => (
              <Badge key={name} variant="outline" className="text-xs">{name}: {amount}</Badge>
            ))}
          </div>
        )}
      </div>
...
export function WorldDetailView({ world, onClose, onEdit }: WorldDetailViewProps) {
  const { t } = useTranslation();
  const dims = world.dimensions;

  const hasDimensions = dims && Object.values(dims).some((v) =>
      ...
      <div className="space-y-6 p-4">
        ...
        {hasDimensions && dims ? (
          <div className="space-y-4">
            ...
            {dims.economy && (
              <EconomySection economy={dims.economy} t={t} />
            )}
            {dims.socialStructure && (
              <SocialSection social={dims.socialStructure} t={t} />
            )}
            {dims.tone && (
              <ToneSection tone={dims.tone} t={t} />
            )}
            {dims.mechanics && (
