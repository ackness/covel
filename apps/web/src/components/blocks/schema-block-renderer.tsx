...
export function SchemaBlockRenderer({ schema, data, onSubmit, disabled }: SchemaBlockRendererProps) {
...
function renderDisplayField(
          ...
          {value.map((item, i) => (
            <div key={i} className="border border-border p-2 text-sm">
              {typeof item === "object" && item !== null
                ? renderObjectFields(item as Record<string, unknown>, itemSchema)
                : String(item)}
            </div>
          ))}
        </div>
  ...
  return (
    <div key={key} className="space-y-1">
      <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">{key}</p>
      <p className="text-sm font-mono text-muted-foreground">{JSON.stringify(value)}</p>
    </div>
  );
}
        ...
        {arrayFields.map(([key, propSchema]) => {
          ...
          return (
            <div key={key} className="space-y-3">
              {items.map((item, idx) => {
                const fieldKey = (item.key as string) ?? (item.id as string) ?? String(idx);
