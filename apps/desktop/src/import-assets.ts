...
async function extractZipSafely(
  ...
  if (cdOffset + cdSize > buf.length) throw new Error("Invalid zip: CD out of range");
  ...
  for (let i = 0; i < totalEntries; i += 1) {
    if (cursor + 46 > buf.length) throw new Error("Invalid zip: CD header truncated");
    if (buf.readUInt32LE(cursor) !== CD_SIG) throw new Error("Invalid zip: bad CD signature");
    ...
    const rawName = buf.slice(cursor + 46, cursor + 46 + nameLen).toString("utf-8");
    ...
    const isDir = safe.endsWith("/") || uncompressedSize === 0 && compressedSize === 0 && safe.endsWith("/");
    ...
    if (localOffset + 30 > buf.length) throw new Error("Invalid zip: LFH truncated");
    if (buf.readUInt32LE(localOffset) !== LFH_SIG) throw new Error("Invalid zip: bad LFH signature");
...
function pluginMdExists(root: string): boolean {
  ...
  for (const entry of fs.readdirSync(runtimesDir, { withFileTypes: true })) {
    if (entry.isDirectory() && isFileSafe(path.join(runtimesDir, entry.name, "PLUGIN.md"))) {
      return true;
    }
  }
  return false;
...
export async function importAsset(
  ...
  if (!sourcePath) return { ok: false, kind, message: "No source path provided" };
  ...
  if (isDirectorySafe(sourcePath)) {
    ...
    if (fs.existsSync(targetDir)) {
      return { ok: false, kind, itemName: name, message: `Already exists: ${name}` };
    }
  ...
  return { ok: false, kind, message: "Unsupported source (expected a folder or a .zip file)" };
}
