...
async function checkpoint(): Promise<BrowserCheckpoint> {
  return exportSessionCheckpoint(store, SESSION_ID, {
    revision: 1,
    actionId: "bootstrap",
  });
}
...
describe("browser checkpoint world authorization", () => {
  ...
  it.each(["world-data", "session-world", "unrelated-world"])(
    "rejects owner-only %s changes before any checkpoint writes",
    async (change) => {
      ...
      const altered = {
        ...
        session: {
          ...before.session,
          ...(change === "session-world" ? { worldId: "other-world" } : {}),
        },
        world:
          change === "session-world"
            ? null
            : {
                ...before.world!,
                ...(change === "unrelated-world"
                  ? { id: "other-world" }
      ...
      expect(await store.getSession(SESSION_ID)).toEqual(before.session);
  ...
  it.each(["included", "omitted"])(
    "allows owner session synchronization with an %s unchanged world without rewriting it",
    async (world) => {
      const before = await checkpoint();
      ...
      const response = await request("browser-checkpoint", {
        checkpoint: {
          ...before,
          messages: [],
          world: world === "included" ? before.world : null,
        },
      });
      expect(response.status).toBe(200);
  ...
  it.each(["self", "operator"])(
    ...
    async (access) => {
      if (access === "self") vi.stubEnv("DEPLOYMENT_TIER", "self");
      const before = await checkpoint();
      const response = await request(
        "browser-checkpoint",
        {
