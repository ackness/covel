...
describe('Working Memory API routes', () => {
  ...
  describe('PUT → GET → DELETE round-trip', () => {
    ...
    it('GET returns all entries for a session', async () => {
      ...
      const res = await app.request(`/api/sessions/${SESSION_ID}/working-memory`);
      expect(res.status).toBe(200);
    ...
    it('PUT + GET full round-trip', async () => {
      ...
      // GET list
      const listRes = await app.request(`/api/sessions/${SESSION_ID}/working-memory`);
      const body = await listRes.json<{ entries: Array<{ scope: string; key: string; value: unknown }> }>();
      const entry = body.entries.find((e) => e.key === 'goal' && e.scope === 'shared');
