...
describe('GET /api/sessions/:id/state — database view', () => {
  ...
  it('exposes characters when any exist', async () => {
    ...
    const res = await app.request(`/api/sessions/${sessionId}/state`);
    const body = await res.json() as { tables: Record<string, { data: Record<string, unknown> }> };
  ...
  it('returns plugin_data table names in stable alphabetical order', async () => {
    const now = new Date().toISOString();
    await store.setPluginDataBatch([
      { id: 'a', sessionId, pluginId: 'core-z', namespace: 'a', key: 'k', value: 1, createdAt: now, updatedAt: now },
      { id: 'b', sessionId, pluginId: 'core-a', namespace: 'a', key: 'k', value: 2, createdAt: now, updatedAt: now },
      { id: 'c', sessionId, pluginId: 'core-a', namespace: 'z', key: 'k', value: 3, createdAt: now, updatedAt: now },
    ]);
