...
function createMockMediaStore(options: MockMediaStoreOptions = {}): {
  ...
  const store: MediaStore = {
    async put(blob, mime) {
      const bytes = blob instanceof Uint8Array ? blob : new Uint8Array(await (blob as Blob).arrayBuffer());
      ...
      return new ReadableStream<Uint8Array>({
        start(controller) {
          ...
          for (let off = 0; off < a.bytes.byteLength; off += CHUNK) {
            controller.enqueue(a.bytes.subarray(off, Math.min(off + CHUNK, a.bytes.byteLength)));
          }
          controller.close();
        },
      });
...
describe('GET /api/media/:id', () => {
  ...
  it('404 when MediaStore does not know the id', async () => {
    ...
    const app = createTestApp(mock.store);
    const res = await app.request(`/api/media/sha-unknown?token=${encodeURIComponent(token)}`);
  ...
  it('503 when mediaStore is not wired (P0-a transitional)', async () => {
    ...
    const res = await app.request(`/api/media/sha-x?token=${encodeURIComponent(token)}`);
