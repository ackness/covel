...
describe("action cancellation during memory recovery", () => {
  it.each(["player", "shutdown"])(
    "settles a %s cancellation while prior memory still finishes durably",
    async (source) => {
      ...
      try {
        ...
        else await applicationWork.close();
        ...
        await sessionLock.withLock(sessionId, async () => {
          expect(
            (await store.getSession(sessionId))?.completedPlayerTurns,
          ).toBe(1);
