...
type Deps = {
  ...
  toolExecutor: { execute: () => Promise<unknown>; getToolInfo: () => undefined };
...
function makeDefaultDeps(store: DataStore, overrides?: Partial<Deps>): Deps {
  ...
  pluginRegistry.register({
    id: 'test-plugin',
    summary: { id: 'test-plugin', name: 'Test Plugin', description: '', version: '0.0.0', pluginType: 'community', source: 'community' },
    manifests: [{ manifest: TEST_MANIFEST, promptTemplate: 'Test prompt', references: [] }],
  ...
  return {
    ...
    toolExecutor: {
      execute: async () => ({ result: '{}', parsedResult: {}, success: true, toolCallId: '', name: '' }),
...
describe('Resume Routes — flag on (COVEL_SUSPEND_V1=1)', () => {
  ...
  describe('POST /api/sessions/:id/resume', () => {
    ...
    it('returns 404 when suspension does not exist', async () => {
      ...
      const res = await app.request('/api/sessions/sess-1/resume', {
        ...
        body: JSON.stringify({ suspensionId: 'nonexistent-susp', data: { name: 'Alice' } }),
      });
    ...
    it('returns 404 when suspension belongs to a different session', async () => {
      ...
      await createSuspension(store, { id: 'susp-other', sessionId: 'sess-other' });
      const app = createTestApp(makeDefaultDeps(store));
      ...
      const res = await app.request('/api/sessions/sess-1/resume', {
        ...
        body: JSON.stringify({ suspensionId: 'susp-other', data: { name: 'Alice' } }),
      });
    ...
    it('returns 400 when resume data has wrong field type', async () => {
      ...
      const res = await app.request('/api/sessions/sess-1/resume', {
        ...
        body: JSON.stringify({ suspensionId: 'susp-1', data: { age: 'not-a-number' } }),
      });
  ...
  describe('DELETE /api/sessions/:id/suspensions/:suspensionId', () => {
    it('returns 404 when suspension does not exist', async () => {
      ...
      const res = await app.request('/api/sessions/sess-1/suspensions/nonexistent', {
        method: 'DELETE',
      });
    ...
    it('removes suspension from store after deletion', async () => {
      ...
      await app.request('/api/sessions/sess-1/suspensions/susp-1', { method: 'DELETE' });
    ...
    it('returns 404 when suspension belongs to a different session', async () => {
      ...
      const res = await app.request('/api/sessions/sess-1/suspensions/susp-other', {
        method: 'DELETE',
      });
  ...
  describe('GET /api/sessions/:id/suspensions', () => {
    ...
    it('does not include suspensions from other sessions', async () => {
      ...
      await createSuspension(store, { id: 'susp-theirs', sessionId: 'sess-other' });
      const app = createTestApp(makeDefaultDeps(store));
