...
function createTestApp(store: DataStore, pluginRegistry: PluginRegistry): Hono<Env> {
...
describe('world routes', () => {
  ...
  it('POST /api/worlds/:id/sync-dimensions refreshes plugin_data and lorebook entries together', async () => {
    ...
    const pluginData = await store.listPluginData('sess-1', 'world-data-provider', 'entries');
    expect(pluginData.map((record) => record.key).sort()).toEqual(['factions', 'geography']);
    expect(pluginData.find((record) => record.key === 'geography')?.value).toEqual(makeDimensions('North', 'Guild').geography);
    ...
    expect(lorebookEntries[0]?.content).toBe('[factions]\n[\n  {\n    "id": "guild",\n    "name": "Guild",\n    "description": "Guild description",\n    "type": "guild",\n    "influence": "minor"\n  }\n]');
    expect(lorebookEntries[1]?.content).toBe('[geography]\n{\n  "regions": [\n    {\n      "name": "North",\n      "description": "North description",\n      "climate": "temperate"\n    }\n  ]\n}');

    const cfg = await loadSessionConfig(store, 'sess-1', 'world-2', 'world-data-provider');
