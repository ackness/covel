 ...
 * emitted zip.
 */
async function buildZipWithRawEntries(
...
describe("POST /api/install/plugin", () => {
	it("requires the desktop bearer token when configured", async () => {
		...
		expect(
			(
				await app.request("/api/install/plugin", {
					method: "POST",
	...
	it("rejects zip-slip entries", async () => {
		...
		expect(body.error.toLowerCase()).toMatch(
			/unsafe|invalid|traversal|relative/,
		);
	});
	...
	it("rejects symlink entries", async () => {
		...
		zip.addBuffer(Buffer.from("/etc/passwd", "utf-8"), "evil-link", {
			mode: 0o120644,
		} as unknown as { mode: number; mtime?: Date });
		const zipBuf = await zipToBuffer(zip);
