...
describe("traceRoutes legacy asset adapter", () => {
  it("synthesizes asset.generated events from legacy plugin_data images", async () => {
    ...
    const legacy = body.events.find(
      (event) => event.type === "asset.generated",
    );
  ...
  it("dedupes synthesized legacy assets when a native asset.generated trace exists", async () => {
    ...
    expect(
      body.events.filter((event) => event.type === "asset.generated"),
    ).toHaveLength(1);
  });
  ...
  it("scans plugin_data rows across all namespaces, not just images", async () => {
    ...
    const asset = legacy?.payload.asset as Record<string, unknown>;
    expect((asset.ref as { url: string }).url).toBe(
      "https://example.test/from-gallery.png",
    );
  });
  ...
  it("caps the legacy plugin_data row scan to bound REST payload size", async () => {
    ...
    const legacyEvents = body.events.filter(
      (event) => event.type === "asset.generated",
    );
