...
describe('Character REST API routes', () => {
  ...
  it('POST returns 400 when required fields are missing', async () => {
    const missingId = await app.request(`/api/sessions/${sessionId}/characters`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: 'No ID' }),
    });
    expect(missingId.status).toBe(400);
    expect(await missingId.json()).toEqual({ error: 'id (string) is required' });

    const missingName = await app.request(`/api/sessions/${sessionId}/characters`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: 'char-missing-name' }),
    });
    expect(missingName.status).toBe(400);
    expect(await missingName.json()).toEqual({ error: 'name (string) is required' });
  });
