...
function makeLoadedRuntime(args: { name: string; pluginId: string; uiSpecs?: unknown }): LoadedRuntime {
  return {
    manifest: {
      name: args.name,
...
describe('GET /api/ui-specs session-aware filter', () => {
  ...
  beforeEach(async () => {
    ...
    registry.register(makeEntry({
      id: 'optional-plugin',
      summary: makeSummary({ id: 'optional-plugin', name: 'Optional', pluginType: 'plugin' }),
      loadedRuntimes: new Map([
        ['optional-plugin', makeLoadedRuntime({
          name: 'optional-plugin',
          pluginId: 'optional-plugin',
          uiSpecs: { right: [{ id: 'opt', label: 'Opt', component: 'Stack', view: {} }] },
        })],
      ]),
    }));
