...
describe('media-token signMediaToken / verifyMediaToken', () => {
  ...
  it('rejects a tampered signature', () => {
    ...
    const verdict = verifyMediaToken(tampered, 'sha-abc', TEST_SECRET, FIXED_NOW);
    expect(verdict.ok).toBe(false);
    if (!verdict.ok) {
  ...
  it('rejects a token whose embedded id does not match the URL parameter', () => {
    ...
    const verdict = verifyMediaToken(token, 'sha-other', TEST_SECRET, FIXED_NOW);
    expect(verdict.ok).toBe(false);
    if (!verdict.ok) {
  ...
  it('rejects a body whose JSON does not match the schema', () => {
    ...
    const evilBody = Buffer.from(JSON.stringify({ id: 'sha-abc', exp: FIXED_NOW + 60_000 }), 'utf8').toString(
      'base64url',
    );
    // Compute a real signature so the failure mode is "malformed" not
    // "bad_signature".
    const sig = createHmac('sha256', TEST_SECRET).update(evilBody).digest('base64url');
  ...
  it('signMediaTokenForSession applies the default 5-minute TTL', () => {
    const tok = signMediaTokenForSession('sha-abc', 'sess-1', undefined, FIXED_NOW);
    // Verify just-in-time before expiry.
    const verdict = verifyMediaToken(tok, 'sha-abc', getMediaTokenSecret(), FIXED_NOW + MEDIA_TOKEN_TTL_MS - 1);
    expect(verdict.ok).toBe(true);
    // And just past expiry.
    const expired = verifyMediaToken(tok, 'sha-abc', getMediaTokenSecret(), FIXED_NOW + MEDIA_TOKEN_TTL_MS + 1);
