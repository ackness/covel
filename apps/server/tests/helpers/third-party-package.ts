...
export async function buildThirdPartyPluginZip(): Promise<Buffer> {
  ...
  const addDirectory = async (relative: string): Promise<void> => {
    const entries = await readdir(path.join(fixtureRoot, relative), {
      withFileTypes: true,
    });
    for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      if (entry.isDirectory()) await addDirectory(name);
        ...
        zip.addBuffer(await readFile(path.join(fixtureRoot, name)), name, {
