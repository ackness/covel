...
import { eventRoutes } from '../../src/routes/v2/events.js';
...
describe('V2 SSE Events', () => {
  ...
  describe('POST /v2/events/emit', () => {
    it('should emit an event and return id', async () => {
      const res = await app.request('/v2/events/emit', {
        method: 'POST',
    ...
    it('should trigger EventBus handlers', async () => {
      ...
      await app.request('/v2/events/emit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          topic: 'test.event',
  ...
  describe('GET /v2/events/subscribe', () => {
    ...
    it('should return SSE content type', async () => {
      const res = await app.request('/v2/events/subscribe?sessionId=sess-1');
      expect(res.status).toBe(200);
