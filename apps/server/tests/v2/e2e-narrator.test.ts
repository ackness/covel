 ...
 *
 * Flow:
 *   POST /v2/session/start → create session, activate core-narrator
 *   POST /v2/session/:id/turn → execute turn with narrator
 *   GET /v2/session/:id/results → verify narrative output
 *   GET /v2/session/:id/turns → verify turn history
 */
...
import { bootstrapV2 } from '../../src/routes/v2/bootstrap.js';
...
describe('E2E: Narrator game flow via V2 API', () => {
  ...
  it('should complete a full game turn through the API', async () => {
    ...
    // 2. Execute a turn
    const turnRes = await app.request(`/v2/session/${sessionId}/turn`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    // 3. Get results
    const resultsRes = await app.request(`/v2/session/${sessionId}/results`);
  ...
  it('should return 404 for turn on non-existent session', async () => {
    const res = await app.request('/v2/session/nonexistent/turn', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
