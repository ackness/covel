...
async function ensureSession(
  ...
  if (!session) {
    return { ok: false, status: 404, body: { error: `Session not found: ${sessionId}` } };
  }
  return { ok: true };
}
...
runtimeOutputRoutes.get('/:id/runtime-outputs/:outputId/full-prompt', async (c) => {
  const store = c.get('store');
  const sessionId = c.req.param('id');
  const outputId = c.req.param('outputId');
  const check = await ensureSession(store, sessionId);
  ...
  const meta = record.metaData as { rawPromptDelta?: Array<{ role: string; content: string }> } | null;
  ...
  return c.json({
    ...
    note:
      'Best-effort rebuild: history comes from turn_messages, delta is stored on RuntimeOutput.metaData.rawPromptDelta.',
  });
});
