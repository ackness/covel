...
function validatePluginAccess(
  ...
  if (requireActive) {
    ...
    if (!isActive) {
      return { error: `Plugin "${pluginId}" is not active in this session`, status: 403 };
    }
  }
  return null;
}
...
pluginDataRoutes.get('/:id/plugin-data/:pluginId/:namespace/:key', async (c) => {
  const store = c.get('store');
  const registry = c.get('pluginRegistry');
  const sessionId = c.req.param('id');
  const session = await store.getSession(sessionId);
  ...
  const record = await store.getPluginData(sessionId, pluginId, namespace, key);
  if (!record) {
...
pluginDataRoutes.put('/:id/plugin-data/:pluginId/:namespace/:key', async (c) => {
  const store = c.get('store');
  const registry = c.get('pluginRegistry');
  const sessionId = c.req.param('id');
  const session = await store.getSession(sessionId);
...
pluginDataRoutes.delete('/:id/plugin-data/:pluginId/:namespace/:key', async (c) => {
  const store = c.get('store');
  const registry = c.get('pluginRegistry');
  const sessionId = c.req.param('id');
  const session = await store.getSession(sessionId);
