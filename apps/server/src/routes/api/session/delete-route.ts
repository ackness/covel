...
/** Shared lifecycle deletion for the session endpoint and world cascade. */
export async function deleteSessionWithLifecycle(
  c: Context,
  id: string,
  expectedSession?: SessionRecord,
): Promise<Response> {
  const store = c.get("store");
  const guard = await resolveSessionById(c, id);
  ...
  const expectedIncarnation = sessionIncarnationIdentity(
...
export function registerSessionDeleteRoute(
  ...
  routes.delete("/:id", (c) =>
    deleteSessionWithLifecycle(c, c.req.param("id")),
  );
}
