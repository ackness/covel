...
export async function loadSessionConfig(
  ...
  try {
    ...
    if (worldDataPluginId) {
      const schemaRecords = await store.listPluginData(
        sessionId,
        worldDataPluginId,
        "schema",
      );
      ...
      const lorebookEntries = await readLorebookWorldEntries(
        store,
        sessionId,
        worldDataPluginId,
      );
      if (lorebookEntries !== undefined) {
        ...
        const entryRecords = await store.listPluginData(
          sessionId,
          worldDataPluginId,
          "entries",
        );
      ...
      if (world?.metadata) {
        ...
        if (dims) {
          ...
          if (typeof dims === "object" && dims !== null) {
            ...
            const sc = d.startingConditions as
              | Record<string, unknown>
              | undefined;
            if (sc?.openingScenario)
              configData.worldOpeningScenario = sc.openingScenario;
          }
        }
      }
