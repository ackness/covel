...
export function createWorldFileWatcher(
  ...
  async function reloadWorld(worldId: string) {
    ...
    try {
      ...
      const oldDims = (existing.metadata as Record<string, unknown> | undefined)?.dimensions;
      const newDims = (newRecord.metadata as Record<string, unknown> | undefined)?.dimensions;
  ...
  async function notifySessions(worldId: string, changedKeys: string[]) {
    try {
      ...
      console.warn(`[world-watcher] Failed to notify sessions for ${worldId}:`, err);
    }
  }
  ...
  return {
    start() {
      ...
      if (process.platform === 'linux') {
        console.log(`[world-watcher] Skipped — recursive fs.watch not supported on Linux. Use POST /api/worlds/:id/dimensions/import for updates.`);
        return;
      }

      try {
        watcher = watch(worldsDir, { recursive: true }, (_eventType, filename) => {
          if (!filename) return;
