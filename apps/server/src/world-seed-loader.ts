 ...
 */
function resolveSafePath(worldDir: string, relativePath: string): string | null {
...
async function resolveLocaleDimensionPath(
  ...
  if (lang) {
    ...
    const localePath = path.join(parsed.dir, `${parsed.name}.${lang}${parsed.ext}`);
...
async function loadExternalDimensions(
  ...
  for (const [key, relativePath] of Object.entries(sources)) {
    ...
    if (!DIMENSION_KEYS.includes(key)) {
      console.warn(`[world-seed] ${worldId}: unknown dimension key "${key}" in dimensionSources, skipping`);
      continue;
    }

    // Path traversal check on the declared path
    if (!resolveSafePath(worldDir, relativePath)) {
      console.warn(`[world-seed] ${worldId}: path traversal detected for "${key}": ${relativePath}, skipping`);
      continue;
    }

    // Resolve with locale awareness
    const resolvedPath = await resolveLocaleDimensionPath(worldDir, relativePath, defaultLocale);
    if (!resolvedPath) {
      console.warn(`[world-seed] ${worldId}: dimension file not found for "${key}": ${relativePath}`);
      continue;
    }
    ...
    try {
      ...
      console.warn(`[world-seed] ${worldId}: failed to load dimension file "${relativePath}":`, err);
    }
  }
 ...
 */
export async function loadSingleWorld(worldDir: string): Promise<WorldRecord | null> {
  ...
  const dimensionSources = manifest.dimensionSources as Record<string, string> | undefined;
  ...
  const externalDims = dimensionSources
    ? await loadExternalDimensions(worldDir, dimensionSources, worldId, defaultLocale)
    : {};
