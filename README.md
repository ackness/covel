...
covel/
├── apps/
│   ├── web/          前端 V1（React 19 + Vite 8，legacy）
│   ├── web-v2/       前端 V2（插件驱动 UI + json-render，活跃开发）
│   ├── desktop/      Electron 桌面应用
│   └── server/       Hono API 服务器 + Drizzle ORM
├── packages/
│   ├── shared/       共享类型与契约
│   ├── runtime/      执行引擎（LLM tool-calling loop）
│   ├── context/      上下文构建（TurnContextStore + PromptAssembler）
│   ├── ai-provider/  多 Provider LLM 抽象（2597 模型数据库）
...
├── plugins/          核心插件（pregame / narrator / codex / npc-graph / …）
...
| [前端面板](docs/reference/ui-panels.md) | V2 插件驱动 UI 架构（json-render）|
