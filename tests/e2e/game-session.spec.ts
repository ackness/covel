...
test.describe("Game Session — 3 Round Flow", () => {
  ...
  test("3 rounds: char creation → message → message, guide triggers", async ({ page }) => {
    // ── Setup ────────────────────────────────────────────────
    await page.goto("/");
    await page.evaluate(() => {
    ...
    const startButton = page.locator("button", { hasText: /start game|开始游戏/i });
    await expect(startButton).toBeVisible({ timeout: 10_000 });
    await startButton.click();

    // Wait for game view
    await expect(page.locator("input[type=text]").last()).toBeVisible({ timeout: 30_000 });
    ...
    await page.screenshot({ path: "tests/e2e/artifacts/game-r0-session-start.png" });
    ...
    await waitForExecDone(page);
    await page.screenshot({ path: "tests/e2e/artifacts/game-r1-char-created.png" });
    ...
    if (clickedGuide) {
      await page.screenshot({ path: "tests/e2e/artifacts/game-r3-guide-clicked.png" });
    } else {
...
async function detectGuideOutput(page: Page): Promise<boolean> {
  ...
  const choiceButtons = page.locator("main button[data-variant=ghost][class*=justify-start]");
  ...
  const colorDivs = page.locator("main div[class*=border-green-500], main div[class*=border-red-500], main div[class*=border-amber-500], main div[class*=border-purple-500]");
  if (await colorDivs.count() > 0) return true;

  // action_guide: categorized suggestions with style labels
  const actionGuide = page.locator("text=/稳妥|激进|创意|狂野|疯狂|safe|aggressive|creative|wild/i");
...
async function tryClickGuideOption(page: Page): Promise<boolean> {
  ...
  const choiceBtn = page.locator("main button[data-variant=ghost][class*=justify-start]").first();
