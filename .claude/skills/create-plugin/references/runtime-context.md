...
export default async function handler(ctx: FunctionHandlerContext) {
  // ctx 的全字段见下方
  return { /* normalized output, 见末尾「返回值」一节 */ };
}
```
...
```ts
const ref = await ctx.media.ingestUrl('https://provider.example.com/output/xxx.png', {
...
### 推荐流程：bytes → MediaStore → assets[] → frontend
...
// 3. 同时 plugin-data 索引 + assets[] 触发 asset.generate proposal
return {
  pluginData: [{
    namespace: 'tracks',
    key: ctx.turnId,
...
export default async function handler(ctx) {
  ...
  return {
    ...
    assets: [{ ref, modality: 'audio', meta: { turnId } }],
  };
}
```
