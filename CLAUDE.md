...
- **Scheduling declaration surface (new, compat period)**: manifests now double-declare a named-stage + typed-dependency surface (`stage` / `after` / `needs` / `inputs`, plus function `ctx.inputs` / `ctx.exports` / `ctx.activation` / `ctx.execution` / `ctx.progress.report`) alongside the legacy `priority` / `upstreamRequired`. Production scheduling still reads the legacy priority bands (`priority` normalized to `legacyOrder`); the new fields are declared and mostly observe-only until Step 6 collapses the old surface. Author-facing reference: [docs/guide/plugin-authoring.md](./docs/guide/plugin-authoring.md#调度声明新).
...
These numeric bands remain the consumed ordering surface during the compat period; the new named `stage` axis (`setup` / `pre-turn` / `narrative` / `post-turn` / `audit`) is double-declared on manifests and maps 1:1 onto these ranges — see the scheduling-declaration bullet above and [docs/guide/plugin-authoring.md](./docs/guide/plugin-authoring.md#调度声明新).
