...
- **Scheduling declaration surface**: manifests declare a named `stage` (`setup` / `pre-turn` / `narrative` / `post-turn` / `audit`) plus typed dependency edges — `needs` (turn-scoped upstream gate + same-pass DAG edge), `after` (same-pass ordering only, no gate), and `inputs` bindings (plus legacy `input.inject` runtime injects) — and for function runtimes, `ctx.inputs` / `ctx.exports` / `ctx.activation` / `ctx.execution` / `ctx.progress.report`. This is the sole scheduling authority; the old numeric priority scheduler (`scheduleByPriority`) is deleted. `priority` / `upstreamRequired` / `jobStatus` are **rejected by both manifest schemas** — a PLUGIN.md declaring any of them fails to load (`parse-plugin-md.ts` keeps a tailored hint pointing at `stage` / `needs`); the compat fold (`stageForPriority`, `aliasUpstreamRequired`) is gone. **Every bundled plugin single-declares `stage` + `needs`/`after`** (the former pregame / world-init exceptions migrated to `stage: setup` + `trigger: auto`, with schema-gen carrying an authored `after: [pregame]` edge; the conservative legacy-order setup chain is deleted — intra-stage order comes exclusively from declared edges). Author-facing reference: [docs/guide/plugin-authoring.md](./docs/guide/plugin-authoring.md#调度声明).
